/*
FILE: source/dominium/apps/workbench/module/domain/transport/main_transport_editor.cpp
MODULE: Dominium
LAYER / SUBSYSTEM: Dominium impl / apps/workbench/module/domain/transport/main_transport_editor
RESPONSIBILITY: Implements `main_transport_editor`; owns translation-unit-local helpers/state; does NOT define the public contract (see `include/**`).
...
static void print_usage(void) {
    std::printf("Usage: dominium-transport-editor [--home=<path>] [--load=<path>] [--demo]\n");
}
...
int main(int argc, char **argv) {
    ...
    {
        const u32 focus[] = { D_TLV_SCHEMA_SPLINE_V1 };
        dom::tools::DomContentToolController controller(
            "transport_editor",
            "Transport Editor",
            "Edit spline transport profiles (speed/capacity/rules).",
            &focus[0],
            sizeof(focus) / sizeof(focus[0]),
            "data/tools_demo/transport_demo.tlv"
        );
