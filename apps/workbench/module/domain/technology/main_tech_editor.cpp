/*
FILE: source/dominium/apps/workbench/module/domain/technology/main_tech_editor.cpp
MODULE: Dominium
LAYER / SUBSYSTEM: Dominium impl / apps/workbench/module/domain/technology/main_tech_editor
RESPONSIBILITY: Implements `main_tech_editor`; owns translation-unit-local helpers/state; does NOT define the public contract (see `include/**`).
...
static void print_usage(void) {
    std::printf("Usage: dominium-tech-editor [--home=<path>] [--load=<path>] [--demo]\n");
}
...
int main(int argc, char **argv) {
    ...
    {
        const u32 focus[] = { D_TLV_SCHEMA_RESEARCH_V1 };
        dom::tools::DomContentToolController controller(
            "tech_editor",
            "Tech Tree Editor",
            "Edit research graphs (nodes+prereqs) with cycle checks.",
            &focus[0],
            sizeof(focus) / sizeof(focus[0]),
            "data/tools_demo/tech_demo.tlv"
        );
