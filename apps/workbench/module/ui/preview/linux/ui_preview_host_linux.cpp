/*
FILE: apps/workbench/module/ui/preview/linux/ui_preview_host_linux.cpp
MODULE: Dominium tools
RESPONSIBILITY: Linux UI preview host (GTK/native DUI backend + hot reload).
*/
#include <stdio.h>
#include <string.h>
#include <string>
...
int main(int argc, char** argv)
{
    (void)argc;
    (void)argv;
    printf("dominium-ui-preview-host-linux: not supported on this platform.\n");
    return 0;
}
...
extern "C" const void* dom_dui_gtk_get_api(u32 requested_abi);
...
{
    printf("Dominium UI Preview Host (Linux)\n");
    printf("Usage:\n");
    printf("  dominium-ui-preview-host-linux --ui <path/to/ui_doc.tlv> [--targets <list>] [--watch] [--log <path>]\n");
    printf("Options:\n");
...
static bool preview_init_backend(PreviewHost& host)
{
    ...
    host.api = (const dui_api_v1*)dom_dui_gtk_get_api(DUI_API_ABI_VERSION);
    host.use_null = 0;
    if (!host.api) {
        host.log.line("backend: gtk api unavailable");
        return false;
    }
    ...
    desc.title = "Dominium UI Preview Host (Linux)";
    desc.width = 1024;
    desc.height = 720;
    if (host.api->create_window(host.ctx, &desc, &host.win) != DUI_OK) {
        host.log.line("backend: gtk create_window failed");
