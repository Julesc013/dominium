/*
FILE: source/dominium/apps/workbench/module/process/editor/main_process_editor.cpp
MODULE: Dominium
LAYER / SUBSYSTEM: Dominium impl / apps/workbench/module/process/editor/main_process_editor
RESPONSIBILITY: Implements `main_process_editor`; owns translation-unit-local helpers/state; does NOT define the public contract (see `include/**`).
...
static void print_usage(void) {
    std::printf("Usage: dominium-process-editor [--home=<path>] [--load=<path>] [--demo]\n");
}
...
int main(int argc, char **argv) {
    ...
    {
        const u32 focus[] = { D_TLV_SCHEMA_PROCESS_V1 };
        dom::tools::DomContentToolController controller(
            "process_editor",
            "Process Editor",
            "Edit recipes and IO graphs (deterministic TLV output).",
            &focus[0],
            sizeof(focus) / sizeof(focus[0]),
            "data/tools_demo/process_demo.tlv"
        );
