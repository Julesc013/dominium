...
int WINAPI wWinMain(HINSTANCE inst, HINSTANCE prev, PWSTR cmdline, int show)
{
    ...
    domui_win32_load_ui_ir("launcher/ui");

    cmd = appcore_ui_command_desc_for_action("launcher.nav.play");
    (void)appcore_dispatch_command(cmd);

    return domui_win32_run_shell(L"Dominium Launcher");
}
