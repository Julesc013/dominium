...
/* dom_snapshot_query_desc: Generic query envelope (opaque payloads). */
typedef struct dom_snapshot_query_desc {
    u32        query_id;
    const void* in;
    u32        in_size;
    void*      out;
    u32        out_size;
} dom_snapshot_query_desc;
...
int dom_snapshot_query(const dom_snapshot_handle* snapshot,
                       const dom_snapshot_query_desc* query);
