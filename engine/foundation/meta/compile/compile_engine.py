...
def _class_default_owner_id(owner_id: str) -> str:
    ...
    if token.startswith("compiled.") or token.startswith("compiled_model."):
...
def build_state_vector_definition_row(
    *,
    ...
    for row in sorted(
        (dict(item) for item in _as_list(state_fields) if isinstance(item, Mapping)),
...
def normalize_state_vector_definition_rows(rows: object) -> List[dict]:
    ...
    for row in sorted(
        ...
        owner = str(normalized.get("owner_id", "")).strip()
        if owner:
            out[owner] = dict(normalized)
...
def serialize_state(
    *,
    ...
    for field in sorted(
        ...
        serialized[field_id] = _normalize_value(source.get(path) if path in source else field.get("default"))
    snapshot = build_state_vector_snapshot_row(
        ...
        extensions=dict(
            _as_map(extensions),
            owner_definition_fingerprint=str(definition.get("deterministic_fingerprint", "")).strip(),
        ),
    )
    return {
        "result": "complete",
    ...
    *,
    ...
    for field in sorted(
        ...
        if not field_id:
            continue
        path = str(field.get("path", "")).strip() or field_id
        restored[path] = _normalize_value(serialized.get(field_id) if field_id in serialized else field.get("default"))
    return {
        "result": "complete",
        "reason_code": "",
        "owner_id": owner_token,
