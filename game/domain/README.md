...
`game/domain/` owns Dominium-specific domain implementation: rules, process semantics, deterministic domain engines, and domain-owned simulation behavior.
...
Root-level domain folders retired in CONVERGE-09 must not be recreated as active source roots. New domain material must be split by ownership across `contracts/`, `game/domain/`, `content/`, `docs/domains/`, and `tests/`.
