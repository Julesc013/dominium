...
from game.domain.geology.index.object_id_engine import geo_object_id
...
def _earth_bindings() -> dict:
    from game.domain.worldgen.earth.climate_field_engine import (
        DEFAULT_EARTH_CLIMATE_PARAMS_ID,
        earth_climate_params_rows,
        evaluate_earth_tile_climate,
    )
    from game.domain.worldgen.earth.hydrology_engine import (
    ...
    from game.domain.worldgen.earth.tide_field_engine import (
        DEFAULT_TIDE_PARAMS_ID,
        evaluate_earth_tile_tide,
        tide_params_rows,
    )
    from game.domain.worldgen.earth.material import evaluate_earth_tile_material_proxy
    surface_module = importlib.import_module("game.domain.worldgen.earth." + "earth" + "_surface" + "_generator")
...
def earth_surface_params_rows(payload: Mapping[str, object] | None = None) -> Dict[str, dict]:
    return _rows_by_id(
        _as_map(payload) or _registry_payload(EARTH_SURFACE_PARAMS_REGISTRY_REL),
        row_key="earth_surface_params",
        id_key="params_id",
    )
...
def _longitude_mdeg(tile_cell_key: Mapping[str, object], surface_priors_row: Mapping[str, object]) -> int:
    cell_key = _coerce_cell_key(tile_cell_key) or {}
    atlas_resolution = max(1, _as_int(_as_map(_as_map(surface_priors_row).get("extensions")).get("atlas_resolution", 8), 8))
    refinement_level = max(0, _as_int(cell_key.get("refinement_level", 0), 0))
    column_count = max(4, atlas_resolution * 4 * max(1, 1 << refinement_level))
    index_tuple = list(cell_key.get("index_tuple") or [0, 0])
    u_idx = _as_int(index_tuple[0] if index_tuple else 0, 0) % column_count
    midpoint = (u_idx * 2) + 1
    return -180_000 + ((midpoint * 360_000) // (column_count * 2))
...
def _default_surface_tile_plan(
    *,
    generator_seed: str,
    ...
    return {
        ...
        "extensions": {
            ...
            "surface_class_id": "surface.class.{}".format(material_key),
            "far_lod_visual_class": (
                "visual.class.blue_ocean"
                if material_key == "ocean"
                else "visual.class.white_polar" if material_key == "ice" else "visual.class.green_brown_land"
            ),
...
def generate_mw_surface_l3_payload(
    *,
    ...
    tide_params_registry_payload: Mapping[str, object] | None = None,
) -> dict:
    ...
    if handler_id == "earth.surface.stub":
        ...
        if not earth_surface_params_row:
            payload = {
                "result": "refused",
                "message": "earth surface generator route is missing declared earth surface parameters",
                "details": {
        ...
        if not tide_params_row:
            payload = {
                "result": "refused",
                "message": "earth surface generator route is missing declared tide parameters",
                "details": {
                    "surface_priors_id": surface_priors_id,
    ...
    if handler_id == "earth.surface.stub" and tide_params_row:
        field_layers.append(
            build_field_layer(
                field_id="field.tide_height_proxy.surface.{}".format(planet_object_token),
    ...
    if handler_id == "earth.surface.stub" and tide_params_row:
        field_initializations.append(
            build_field_cell(
                field_id="field.tide_height_proxy.surface.{}".format(planet_object_token),
                value=int(_as_int(tide_evaluation.get("tide_height_value", 0), 0)),
                last_updated_tick=max(0, _as_int(current_tick, 0)),
                geo_cell_key=surface_cell_key,
                extensions={
                    "field_type_id": "field.tide_height_proxy",
                    "planet_object_id": planet_object_token,
                    "tide_params_id": tide_params_id,
                    "initialization_kind": "earth3_surface_tide",
                    "source": "EARTH3-4",
                },
            )
        )
    ...
    payload = {
        ...
        "surface_summary": {
            ...
            "tide_height_value": int(_as_int(tide_evaluation.get("tide_height_value", 0), 0)),
            "lunar_phase": int(_as_int(tide_evaluation.get("lunar_phase", 0), 0)),
            "rotation_phase": int(_as_int(tide_evaluation.get("rotation_phase", 0), 0)),
            "hydrology_window_fingerprint": str(hydrology_payload.get("window_fingerprint", "")).strip(),
            "current_tick": int(max(0, _as_int(current_tick, 0))),
            "deterministic_fingerprint": "",
        },
