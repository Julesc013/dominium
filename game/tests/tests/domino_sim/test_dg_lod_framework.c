/*
FILE: tests/domino_sim/test_dg_lod_framework.c
MODULE: Repository
...
static int test_budget_deferral(void) {
...

    {
        ...
        /* Initial states are chosen so each desired transition is 1 ladder step (cost=1). */
        test_world_add_obj(&w, 1u, 10u, 1u, 1u, p_hazard, CLASS_ENTITY, DG_REP_R1_LITE);
        test_world_add_obj(&w, 2u, 1u, 5u, 1u, p_hazard, CLASS_ENTITY, DG_REP_R1_LITE);
        test_world_add_obj(&w, 1u, 10u, 2u, 1u, p_player, CLASS_ENTITY, DG_REP_R2_AGG);
        test_world_add_obj(&w, 1u, 10u, 4u, 1u, p_player, CLASS_ENTITY, DG_REP_R2_AGG);
        test_world_add_obj(&w, 2u, 5u, 1u, 1u, p_player, CLASS_ENTITY, DG_REP_R2_AGG);
