...
int main(int argc, char **argv) {
    ...
    }
    {
        ...
        files.push_back(join_path(root, "source/dominium/setup/frontends/adapters/windows_msi/wix/DominiumSetup.wxs"));
    ...
    for (i = 0u; i < files.size(); ++i) {
        if (!file_contains(files[i], "dominium-setup")) {
            std::fprintf(stderr, "missing dominium-setup in: %s\n", files[i].c_str());
