...
static std::string dsk_find_setup2_cli(int argc, char **argv) {
    const char *override_path = dsk_get_arg_value(argc, argv, "--setup2-cli");
    ...
    }
    {
        ...
        if (len > 0u && len < MAX_PATH) {
            ...
            std::string sibling = dsk_join_path(dir, "dominium-setup2.exe");
            if (dsk_file_exists(sibling)) {
                return sibling;
            }
        }
    }
    return "dominium-setup2";
}
...
static void dsk_print_json_summary(const char *command,
    ...
    std::printf("{\"schema_version\":\"setup2-adapter-1\",");
...
static void dsk_print_usage(void) {
    std::printf("dominium-setup2-win-exe --cli|--tui|--gui <request-make|run> [options]\n");
...
int main(int argc, char **argv) {
    ...
    std::string frontend_id = dsk_get_arg_value(argc, argv, "--frontend-id") ? dsk_get_arg_value(argc, argv, "--frontend-id") : "dominium-setup2-win-exe";
    ...
    if (std::strcmp(subcommand, "request-make") == 0) {
        std::string cli = dsk_find_setup2_cli(argc, argv);
