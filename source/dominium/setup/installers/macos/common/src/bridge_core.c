/*
FILE: source/dominium/setup/installers/macos/common/src/bridge_core.c
MODULE: Dominium Setup macOS
PURPOSE: Bridge to Setup Core CLI from macOS frontends.
*/
...
int dsu_macos_bridge_spawn(const char *cmdline, int quiet) {
    int exit_code = -1;
    (void)quiet;
...
int dsu_macos_bridge_export_invocation(const dsu_macos_bridge_paths_t *paths,
                                       const char *args,
                                       int deterministic,
                                       int quiet,
                                       int format_json) {
    ...
    if (format_json) {
        ...
        code = dsu_macos_bridge_spawn(sb.buf, quiet);
        free(sb.buf);
        free(cmd);
        return code;
    }
    code = dsu_macos_bridge_spawn(cmd, quiet);
    free(cmd);
    return code;
}

int dsu_macos_bridge_plan(const dsu_macos_bridge_paths_t *paths,
...
int dsu_macos_bridge_apply_plan(const dsu_macos_bridge_paths_t *paths,
                                const char *plan_path,
                                int deterministic,
                                int dry_run,
                                int quiet,
...
int dsu_macos_bridge_apply_invocation(const dsu_macos_bridge_paths_t *paths,
                                      const char *invocation_path,
                                      int deterministic,
                                      int dry_run,
                                      int quiet,
