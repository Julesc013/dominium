/*
FILE: source/dominium/setup/installers/windows/exe/common/src/invocation_builder.c
...
int dsu_exe_csv_list_parse(const char *csv, dsu_exe_csv_list_t *out_list) {
    ...
    while (*p) {
        ...
        if (*token) {
            ...
            if (!dup) {
                free(tmp);
                dsu_exe_csv_list_free(out_list);
                return 0;
            }
            if (count == cap) {
...
int dsu_exe_build_invocation(const dsu_exe_cli_args_t *args,
    ...
    dsu_exe_csv_list_t components;
    dsu_exe_csv_list_t exclude;
    ...
    out_invocation->platform_triple = _strdup(platform ? platform : "win32-x86");
    out_invocation->ui_mode = _strdup(ui_mode ? ui_mode : "cli");
    out_invocation->frontend_id = _strdup(frontend_id ? frontend_id : "exe-win32");
    ...
    if (args->components_csv && !dsu_exe_csv_list_parse(args->components_csv, &components)) {
        dsu_invocation_destroy(NULL, out_invocation);
        return 0;
    }
    if (args->exclude_csv && !dsu_exe_csv_list_parse(args->exclude_csv, &exclude)) {
        dsu_exe_csv_list_free(&components);
        dsu_invocation_destroy(NULL, out_invocation);
        return 0;
    }
