/*
FILE: source/dominium/setup/installers/windows/exe/common/src/bridge_core.c
MODULE: Dominium Setup EXE
PURPOSE: Bridge to modern Setup Core CLI from EXE frontends.
*/
...
int dsu_exe_bridge_spawn(const char *cmdline, int quiet) {
    ...
    if (!CreateProcessA(NULL, cmd_copy, NULL, NULL, FALSE, 0, NULL, NULL, &si, &pi)) {
        dsu_exe_log_error("failed to spawn: %s", cmdline);
        free(cmd_copy);
        return -1;
    }
...
int dsu_exe_bridge_export_invocation(const dsu_exe_bridge_paths_t *paths,
    ...
    if (format_json) {
        ...
        code = dsu_exe_bridge_spawn(sb.buf, quiet);
        free(sb.buf);
        free(cmd);
        return code;
    }
    code = dsu_exe_bridge_spawn(cmd, quiet);
    free(cmd);
    return code;
}
