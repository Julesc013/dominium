/*
FILE: source/dominium/setup/installers/windows_legacy/win9x/src/win9x_tui.c
MODULE: Dominium Setup (Win9x)
PURPOSE: Text-mode wizard for Win9x legacy installer.
*/
#include "win9x_tui.h"
...
static void win9x_ui_init(dsu_win9x_ui_result_t *out) {
    if (!out) return;
    memset(out, 0, sizeof(*out));
    out->operation = (dsu_legacy_u8)DSU_LEGACY_OPERATION_INSTALL;
    out->scope = (dsu_legacy_u8)DSU_LEGACY_SCOPE_USER;
    out->quick_mode = 1;
}

static void win9x_ui_free_list(char **items, dsu_legacy_u32 count) {
    dsu_legacy_u32 i;
    if (!items) return;
    for (i = 0u; i < count; ++i) {
        free(items[i]);
...
void dsu_win9x_ui_free(dsu_win9x_ui_result_t *out) {
    ...
    win9x_ui_free_list(out->selected_components, out->selected_component_count);
    win9x_ui_free_list(out->excluded_components, out->excluded_component_count);
    win9x_ui_init(out);
}

static int win9x_ui_list_push(char ***items, dsu_legacy_u32 *io_count, dsu_legacy_u32 *io_cap, const char *s) {
...
static int win9x_ui_read_line(char *buf, size_t cap) {
...
static int win9x_ui_prompt_number(const char *prompt, int min, int max, int def_value) {
    char buf[64];
    int v;
    for (;;) {
        printf("%s [%d]: ", prompt, def_value);
        if (!win9x_ui_read_line(buf, sizeof(buf))) return def_value;
...
static int win9x_ui_prompt_yesno(const char *prompt, int def_yes) {
    char buf[16];
    for (;;) {
        printf("%s [%s]: ", prompt, def_yes ? "Y" : "N");
        if (!win9x_ui_read_line(buf, sizeof(buf))) return def_yes;
...
static void win9x_ui_prompt_path(const char *prompt, const char *def_path, char *out, size_t cap) {
    char buf[260];
    printf("%s [%s]: ", prompt, def_path ? def_path : "");
    if (win9x_ui_read_line(buf, sizeof(buf)) && buf[0] != '\0') {
...
static void win9x_ui_select_components(const dsu_legacy_manifest_t *m,
                                       dsu_win9x_ui_result_t *out) {
    dsu_legacy_u32 i;
    char buf[256];
    if (!m || !out) return;
    ...
    }
    {
        ...
        while (*p) {
            ...
            if (idx > 0 && (dsu_legacy_u32)idx <= m->component_count) {
                ...
                if (id && id[0]) {
                    win9x_ui_list_push(&out->selected_components,
...
int dsu_win9x_ui_collect(const char *manifest_path, dsu_win9x_ui_result_t *out) {
    ...
    printf("Dominium Win9x Setup\n");
    printf("====================\n\n");
    printf("1) Install/Upgrade\n");
    printf("2) Repair\n");
    printf("3) Uninstall\n");
    printf("4) Verify\n");
    choice = win9x_ui_prompt_number("Select operation", 1, 4, 1);
    ...
    out->quick_mode = win9x_ui_prompt_yesno("Use Quick Install", 1);

    {
        char path_buf[260];
        win9x_ui_prompt_path("Install path", "C:\\Program Files\\Dominium", path_buf, sizeof(path_buf));
        out->install_root = (char *)malloc(strlen(path_buf) + 1u);
        if (!out->install_root) {
            dsu_win9x_ui_free(out);
            return 0;
        }
    ...
    if (!out->quick_mode && manifest_path && manifest_path[0]) {
        if (dsu_legacy_manifest_load(manifest_path, &manifest) == DSU_LEGACY_STATUS_SUCCESS) {
            win9x_ui_select_components(manifest, out);
