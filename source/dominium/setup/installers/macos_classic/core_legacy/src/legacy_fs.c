...
#if defined(_WIN32)
    if (_mkdir(path) == 0) return DSU_LEGACY_STATUS_SUCCESS;
    if (errno == EEXIST) return DSU_LEGACY_STATUS_SUCCESS;
#else
    if (mkdir(path, 0755) == 0) return DSU_LEGACY_STATUS_SUCCESS;
    if (errno == EEXIST) return DSU_LEGACY_STATUS_SUCCESS;
#endif
...
static dsu_legacy_status_t dsu_legacy_copy_tree_internal(const char *src_root,
                                                         const char *dst_root,
    ...
    intptr_t h;
    char *pattern;
    if (!src_root || !dst_root) return DSU_LEGACY_STATUS_INVALID_ARGS;
    pattern = dsu_legacy_path_join_native(src_root, "*");
    if (!pattern) return DSU_LEGACY_STATUS_IO_ERROR;
    h = _findfirst(pattern, &ent);
    free(pattern);
    if (h == -1) return DSU_LEGACY_STATUS_IO_ERROR;
    do {
        const char *name = ent.name;
        if (strcmp(name, ".") == 0 || strcmp(name, "..") == 0) continue;
        if (rel_prefix && rel_prefix[0] != '\0') {
            next_rel = dsu_legacy_path_join_rel(rel_prefix, name);
        } else {
            next_rel = dsu_legacy_strdup(name);
        }
        if (!next_rel) {
            _findclose(h);
            return DSU_LEGACY_STATUS_IO_ERROR;
        }
        src_path = dsu_legacy_path_join_native(src_root, name);
        if (!src_path) {
            free(next_rel);
            _findclose(h);
            return DSU_LEGACY_STATUS_IO_ERROR;
        ...
        if (ent.attrib & _A_SUBDIR) {
            st = dsu_legacy_copy_tree_internal(src_path, dst_root, next_rel, cb, user);
        } else {
            dst_path = dsu_legacy_path_join_native(dst_root, next_rel);
            if (!dst_path) {
                free(src_path);
                free(next_rel);
                _findclose(h);
            ...
            st = dsu_legacy_fs_copy_file(src_path, dst_path);
            if (st == DSU_LEGACY_STATUS_SUCCESS && cb) {
                st = cb(next_rel, dst_path, user);
            }
            free(dst_path);
        }
        free(src_path);
    ...
    if (!dir) return DSU_LEGACY_STATUS_IO_ERROR;
...
dsu_legacy_status_t dsu_legacy_fs_copy_tree(const char *src_root,
                                            ...
                                            void *user) {
    if (!src_root || !dst_root) return DSU_LEGACY_STATUS_INVALID_ARGS;
    return dsu_legacy_copy_tree_internal(src_root, dst_root, NULL, cb, user);
}
