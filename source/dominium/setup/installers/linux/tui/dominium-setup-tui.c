/*
FILE: source/dominium/setup/installers/linux/tui/dominium-setup-tui.c
...
#include "dsu_linux_invocation.h"
#include "dsu_linux_log.h"
#include "dsu_linux_tui.h"
...
#define DSU_LINUX_TUI_NAME "dominium-setup-tui"
#define DSU_LINUX_TUI_VERSION "0.0.0"
...
static void dsu__usage(FILE *out) {
    fprintf(out,
            ...
            DSU_LINUX_TUI_NAME, DSU_LINUX_TUI_VERSION,
            DSU_LINUX_TUI_NAME);
}
...
static int dsu__prompt_menu(const char *title,
    ...
    dsu_linux_tui_flush();
    if (!dsu_linux_tui_read_line(buf, sizeof(buf))) {
        return choice;
    }
    dsu_linux_tui_trim(buf);
    if (buf[0] == '\0') {
        return choice;
    }
    parsed = atoi(buf);
...
static int dsu__run_platform_register(const dsu_linux_bridge_paths_t *paths,
    ...
    return (dsu_linux_bridge_spawn(cmd, quiet) == 0);
}

static int dsu__run_platform_unregister(const dsu_linux_bridge_paths_t *paths,
                                        const char *state_path,
                                        int deterministic,
                                        int quiet) {
    char cmd[1024];
...
static int dsu__run_non_interactive(const dsu_linux_cli_args_t *args,
                                    const dsu_linux_bridge_paths_t *paths,
                                    const char *platform,
    ...
    const char *out_inv;
    const char *out_plan;
    dsu_linux_cli_args_t tmp_args = *args;
    ...
    if (!tmp_args.frontend_id) tmp_args.frontend_id = "tui-linux";
    ...
    ok = dsu_linux_build_invocation(&tmp_args, platform, tmp_args.ui_mode, tmp_args.frontend_id, &inv);
    if (!ok) {
        dsu_linux_log_error("failed to build invocation");
        dsu_invocation_destroy(NULL, &inv);
        return 1;
    }
    if (!dsu_linux_write_invocation(&inv, out_inv, &digest)) {
        dsu_linux_log_error("failed to write invocation: %s", out_inv);
        dsu_invocation_destroy(NULL, &inv);
        return 1;
    }
    dsu_linux_log_info("invocation written: %s", out_inv);
    dsu_linux_log_info("invocation digest64: 0x%016llx", (unsigned long long)digest);
    ...
    if (tmp_args.run_plan || tmp_args.run_apply || tmp_args.apply_invocation) {
        if (dsu_linux_bridge_plan(paths,
                                  out_inv,
                                  out_plan,
                                  tmp_args.state_path,
            ...
            dsu_linux_log_error("plan failed");
            dsu_invocation_destroy(NULL, &inv);
            return 1;
        }
    }
    ...
    if (tmp_args.apply_invocation) {
        if (dsu_linux_bridge_apply_invocation(paths, out_inv, tmp_args.deterministic, tmp_args.dry_run, tmp_args.quiet, tmp_args.want_json) != 0) {
            dsu_linux_log_error("apply (invocation) failed");
            dsu_invocation_destroy(NULL, &inv);
            return 1;
        }
    } else if (tmp_args.run_apply || !tmp_args.export_invocation) {
        if (dsu_linux_bridge_apply_plan(paths, out_plan, tmp_args.deterministic, tmp_args.dry_run, tmp_args.quiet, tmp_args.want_json) != 0) {
            dsu_linux_log_error("apply failed");
            dsu_invocation_destroy(NULL, &inv);
            return 1;
        }
    }
...
int main(int argc, char **argv) {
    ...
    dsu_linux_bridge_paths_t paths;
    dsu_linux_ui_state_t ui;
    ...
    if (!dsu_linux_args_parse(argc, argv, &args)) {
        dsu__usage(stderr);
        return 2;
    }
    ...
    if (args.log_path) {
        dsu_linux_log_set_file(args.log_path);
    }
    ...
    if (dsu_ctx_create(&cfg, &cbs, NULL, &ctx) != DSU_STATUS_SUCCESS) {
        dsu_linux_log_error("failed to init setup core");
        return 1;
    }
    ...
    if (dsu_manifest_load_file(ctx, manifest_path, &manifest) != DSU_STATUS_SUCCESS) {
        dsu_linux_log_error("failed to load manifest: %s", manifest_path);
        dsu_ctx_destroy(ctx);
        return 1;
    }

    dsu_linux_ui_state_init(&ui);
    strncpy(install_root, "", sizeof(install_root) - 1u);
    install_root[sizeof(install_root) - 1u] = '\0';
    state_path[0] = '\0';

    if (args.non_interactive || args.use_defaults || !dsu_linux_tui_is_tty()) {
        ...
        paths.adapter_exe_path = dsu__resolve_exe_path(argv[0], "dominium-setup-linux", adapter_path, sizeof(adapter_path));
...

    {
        ...
        ui.install_mode = (mode_choice == 1) ? DSU_LINUX_UI_INSTALL_MODE_CUSTOM : DSU_LINUX_UI_INSTALL_MODE_QUICK;
    }

    if (ui.install_mode == DSU_LINUX_UI_INSTALL_MODE_CUSTOM) {
    ...
    if (!dsu__find_manifest_install_root(manifest, ui.scope, platform_buf, install_root, sizeof(install_root))) {
        dsu_linux_log_error("manifest missing install root for platform %s", platform_buf);
        dsu_manifest_destroy(ctx, manifest);
        dsu_ctx_destroy(ctx);
        return 1;
    }
    ...
    if (ui.install_mode == DSU_LINUX_UI_INSTALL_MODE_CUSTOM) {
        ...
        dsu_linux_tui_flush();
        if (dsu_linux_tui_read_line(prompt, sizeof(prompt))) {
            dsu_linux_tui_trim(prompt);
            if (prompt[0] != '\0') {
    ...
    if (ui.install_mode == DSU_LINUX_UI_INSTALL_MODE_CUSTOM) {
        if (!dsu__select_components(manifest, &ui.selected_components, &ui.selected_count)) {
            dsu_linux_log_error("component selection failed");
            dsu_manifest_destroy(ctx, manifest);
            dsu_ctx_destroy(ctx);
            return 1;
        }
...

    {
        ...
        dsu_linux_cli_args_t call_args;
        dsu_invocation_t inv;
        dsu_u64 digest = 0u;
        char inv_path[512];
        char plan_path[512];
        ...
        call_args.frontend_id = "tui-linux";
        ...
        dsu_invocation_init(&inv);
        if (!dsu_linux_build_invocation(&call_args, platform_buf, "tui", "tui-linux", &inv)) {
            dsu_linux_log_error("failed to build invocation");
            free(components_csv);
            dsu_manifest_destroy(ctx, manifest);
            dsu_ctx_destroy(ctx);
        ...
        if (!dsu_linux_write_invocation(&inv, out_inv, &digest)) {
            dsu_linux_log_error("failed to write invocation");
            free(components_csv);
            dsu_invocation_destroy(NULL, &inv);
            dsu_manifest_destroy(ctx, manifest);
            dsu_ctx_destroy(ctx);
        ...
        fprintf(stdout, "  CLI: dominium-setup export-invocation --manifest \"%s\" --op %s --scope %s --platform %s --install-root \"%s\" --ui-mode tui --frontend-id tui-linux --out \"%s\" --deterministic 1",
                manifest_path,
                call_args.operation,
                call_args.scope,
                platform_buf,
...

        {
            ...
            if (dsu_linux_bridge_plan(&paths, out_inv, out_plan, plan_state, 1, 0, 0) != 0) {
                dsu_linux_log_error("plan failed");
                free(components_csv);
                dsu_invocation_destroy(NULL, &inv);
                dsu_manifest_destroy(ctx, manifest);
        ...
        if (dsu_linux_bridge_apply_plan(&paths, out_plan, 1, 0, 0, 0) != 0) {
            dsu_linux_log_error("apply failed");
            free(components_csv);
            dsu_invocation_destroy(NULL, &inv);
            dsu_manifest_destroy(ctx, manifest);
