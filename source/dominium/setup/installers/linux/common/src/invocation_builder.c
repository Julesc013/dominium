/*
FILE: source/dominium/setup/installers/linux/common/src/invocation_builder.c
...
static char *dsu_linux_strdup(const char *s) {
    size_t n;
    char *p;
    if (!s) return NULL;
    n = strlen(s);
...
int dsu_linux_csv_list_parse(const char *csv, dsu_linux_csv_list_t *out_list) {
    ...
    while (*p) {
        ...
        if (*token) {
            char *dup = dsu_linux_strdup(token);
            char **next;
            if (!dup) {
                free(tmp);
                dsu_linux_csv_list_free(out_list);
                return 0;
            }
            if (count == cap) {
...
static int set_single_list(char ***out_items, dsu_u32 *out_count, const char *value) {
    ...
    if (!items) return 0;
    items[0] = dsu_linux_strdup(value);
    if (!items[0]) {
        free(items);
        return 0;
    }
...
int dsu_linux_build_invocation(const dsu_linux_cli_args_t *args,
    ...
    dsu_linux_csv_list_t components;
    dsu_linux_csv_list_t exclude;
    ...
    out_invocation->platform_triple = dsu_linux_strdup(platform ? platform : "linux-x64");
    out_invocation->ui_mode = dsu_linux_strdup(ui_mode ? ui_mode : "cli");
    out_invocation->frontend_id = dsu_linux_strdup(frontend_id ? frontend_id : "tui-linux");
    ...
    if (args->components_csv && !dsu_linux_csv_list_parse(args->components_csv, &components)) {
        dsu_invocation_destroy(NULL, out_invocation);
        return 0;
    }
    if (args->exclude_csv && !dsu_linux_csv_list_parse(args->exclude_csv, &exclude)) {
        dsu_linux_csv_list_free(&components);
        dsu_invocation_destroy(NULL, out_invocation);
        return 0;
    }
    ...
    dsu_linux_csv_list_free(&components);
    dsu_linux_csv_list_free(&exclude);
    return 1;
}
