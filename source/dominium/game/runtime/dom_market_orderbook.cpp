/*
FILE: source/dominium/game/runtime/dom_market_orderbook.cpp
MODULE: Dominium
LAYER / SUBSYSTEM: Dominium impl / game/runtime/market_orderbook
RESPONSIBILITY: Deterministic order book provider (discrete clearing).
*/
...
static int ob_init(void *state, const dom_market_spec *spec) {
    OrderbookState *s = (OrderbookState *)state;
    if (!s || !spec) {
...
static int ob_submit(void *state,
...
static int ob_cancel(void *state, dom_market_order_id order_id) {
    OrderbookState *s = (OrderbookState *)state;
    size_t i;
    if (!s || order_id == 0ull) {
        return DOM_MARKET_INVALID_ARGUMENT;
...
static int ob_clear(void *state,
                    dom_act_time_t now,
...
static int ob_next_due(void *state, dom_act_time_t *out_tick) {
    OrderbookState *s = (OrderbookState *)state;
    if (!s || !out_tick) {
        return DOM_MARKET_INVALID_ARGUMENT;
    }
...
static void ob_destroy(void *state) {
    OrderbookState *s = (OrderbookState *)state;
    if (!s) {
        return;
    }
...
int dom_market_provider_create_orderbook(dom_market_provider_vtbl *out_vtbl, void **out_state) {
    ...
    out_vtbl->init = ob_init;
    out_vtbl->submit_order = ob_submit;
    out_vtbl->cancel_order = ob_cancel;
    out_vtbl->clear = ob_clear;
    out_vtbl->next_due_tick = ob_next_due;
    out_vtbl->destroy = ob_destroy;
    *out_state = state;
    return DOM_MARKET_OK;
}
