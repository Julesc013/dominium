/*
FILE: source/domino/sim/bus/dg_message_bus.c
MODULE: Domino
LAYER / SUBSYSTEM: Domino impl / sim/bus/dg_message_bus
RESPONSIBILITY: Implements `dg_message_bus`; owns translation-unit-local helpers/state; does NOT define the public contract (see `include/**`).
...
/* Delivery queue keys:
 * We map (tick, dst_id, message_type_id, src_entity, seq) into dg_order_key:
 *   phase       = PH_SENSE (delivery boundary)
 *   domain_id   = tick
 *   chunk_id    = dst_id
 *   entity_id   = message_type_id
 *   component_id= src_entity
 *   type_id     = 0 (reserved)
 *   seq         = hdr.seq
 */
static dg_order_key dg_message_bus_key_from_hdr(const dg_pkt_hdr *hdr) {
    dg_order_key k;
    ...
    k.chunk_id = (dg_chunk_id)hdr->dst_entity;
    k.entity_id = (dg_entity_id)hdr->type_id;
    k.component_id = hdr->src_entity;
    k.type_id = 0u;
    k.seq = hdr->seq;
    return k;
}

static int dg_message_bus_sub_cmp(const dg_message_bus_sub *a, const dg_message_bus_sub *b) {
    if (a->dst_id < b->dst_id) return -1;
    if (a->dst_id > b->dst_id) return 1;
    if (a->message_type_id < b->message_type_id) return -1;
    if (a->message_type_id > b->message_type_id) return 1;
...
static u32 dg_message_bus_sub_upper_bound(const dg_message_bus *bus, const dg_message_bus_sub *key) {
    u32 lo = 0u;
    u32 hi;
    u32 mid;
    if (!bus || !key) {
    ...
    while (lo < hi) {
        int cmp;
        mid = lo + ((hi - lo) / 2u);
        cmp = dg_message_bus_sub_cmp(&bus->subs[mid], key);
        if (cmp <= 0) {
            lo = mid + 1u;
        } else {
            hi = mid;
...
static int dg_message_bus_ensure_sub_capacity(dg_message_bus *bus, u32 need_count) {
    u32 new_cap;
    dg_message_bus_sub *new_subs;
    ...
    new_subs = (dg_message_bus_sub *)realloc(bus->subs, sizeof(dg_message_bus_sub) * (size_t)new_cap);
    if (!new_subs) {
        return -2;
    }
    if (new_cap > bus->sub_capacity) {
        memset(new_subs + bus->sub_capacity, 0, sizeof(dg_message_bus_sub) * (size_t)(new_cap - bus->sub_capacity));
    }
    bus->subs = new_subs;
...
static int dg_message_bus_ensure_record_capacity(dg_message_bus *bus, u32 need_count) {
    u32 new_cap;
    dg_message_bus_record *new_recs;
    ...
    new_recs = (dg_message_bus_record *)realloc(bus->records, sizeof(dg_message_bus_record) * (size_t)new_cap);
    if (!new_recs) {
        return -2;
    }
    if (new_cap > bus->record_capacity) {
        memset(new_recs + bus->record_capacity, 0, sizeof(dg_message_bus_record) * (size_t)(new_cap - bus->record_capacity));
    }
...
static int dg_message_bus_ensure_free_id_capacity(dg_message_bus *bus, u32 need_count) {
    u32 new_cap;
    u32 *new_ids;
...
static int dg_message_bus_grow_deliver_q(dg_message_bus *bus, u32 min_capacity) {
    dg_work_queue new_q;
    u32 new_cap;
    int rc;
...
static void dg_message_bus_sub_range(
    const dg_message_bus *bus,
    dg_entity_id          dst_id,
    ...
    hi = bus->sub_count;
    while (lo < hi) {
        dg_entity_id mid_dst;
        dg_type_id mid_type;
        mid = lo + ((hi - lo) / 2u);
        mid_dst = bus->subs[mid].dst_id;
        mid_type = bus->subs[mid].message_type_id;

        if (mid_dst < dst_id) {
            lo = mid + 1u;
        } else if (mid_dst > dst_id) {
            hi = mid;
        } else if (mid_type < message_type_id) {
            lo = mid + 1u;
        } else {
            hi = mid;
        }
    ...
    start = lo;
    count = 0u;
    while ((start + count) < bus->sub_count) {
        const dg_message_bus_sub *s = &bus->subs[start + count];
        if (s->dst_id != dst_id || s->message_type_id != message_type_id) {
            break;
        }
        count += 1u;
    }
...
static int dg_message_bus_alloc_record_id(dg_message_bus *bus, u32 *out_id) {
    u32 id;
    int rc;
    ...
    id = bus->record_count;
    rc = dg_message_bus_ensure_record_capacity(bus, id + 1u);
    if (rc != 0) {
        return -2;
    }
    bus->record_count += 1u;
...
static void dg_message_bus_free_record(dg_message_bus *bus, u32 id) {
    dg_message_bus_record *r;
    int rc;
    ...
    rc = dg_message_bus_ensure_free_id_capacity(bus, bus->free_record_count + 1u);
    if (rc == 0) {
...
int dg_message_bus_reserve(dg_message_bus *bus, u32 subs_cap, u32 records_cap, u32 deliver_queue_cap) {
    ...
    subs = (dg_message_bus_sub *)0;
    recs = (dg_message_bus_record *)0;
    free_ids = (u32 *)0;

    if (subs_cap != 0u) {
        subs = (dg_message_bus_sub *)malloc(sizeof(dg_message_bus_sub) * (size_t)subs_cap);
        if (!subs) {
            dg_message_bus_free(bus);
            return -2;
        }
        memset(subs, 0, sizeof(dg_message_bus_sub) * (size_t)subs_cap);
    }
    if (records_cap != 0u) {
        recs = (dg_message_bus_record *)malloc(sizeof(dg_message_bus_record) * (size_t)records_cap);
        if (!recs) {
            if (subs) free(subs);
            dg_message_bus_free(bus);
            return -3;
        }
        memset(recs, 0, sizeof(dg_message_bus_record) * (size_t)records_cap);
...
int dg_message_bus_subscribe(
    ...
    if (dst_id == 0u || message_type_id == 0u) {
        return -2;
    }

    memset(&sub, 0, sizeof(sub));
    sub.dst_id = dst_id;
    sub.message_type_id = message_type_id;
    sub.fn = fn;
    sub.user_ctx = user_ctx;
    sub.priority_key = priority_key;
    sub.insert_index = bus->next_sub_insert++;

    rc = dg_message_bus_ensure_sub_capacity(bus, bus->sub_count + 1u);
    if (rc != 0) {
        return -3;
    }

    idx = dg_message_bus_sub_upper_bound(bus, &sub);
    if (idx < bus->sub_count) {
        memmove(&bus->subs[idx + 1u], &bus->subs[idx],
                sizeof(dg_message_bus_sub) * (size_t)(bus->sub_count - idx));
    }
    bus->subs[idx] = sub;
    bus->sub_count += 1u;
    return 0;
...
int dg_message_bus_send(dg_message_bus *bus, const dg_pkt_message *msg) {
    ...
    if (msg->payload_len != msg->hdr.payload_len) {
        return -2;
    }
    if (msg->payload_len != 0u && !msg->payload) {
        return -3;
    }
    if (msg->hdr.dst_entity == 0u) {
        return -4; /* addressed delivery requires an explicit destination id */
    }

    if (dg_type_registry_count(&bus->type_registry) != 0u) {
        type_entry = dg_type_registry_find(&bus->type_registry, msg->hdr.type_id, msg->hdr.schema_id, msg->hdr.schema_ver);
        if (!type_entry) {
            return -5;
        }
        if (type_entry->validate_fn) {
            rc = type_entry->validate_fn(msg->hdr.type_id, msg->hdr.schema_id, msg->hdr.schema_ver, msg->payload, msg->payload_len);
            if (rc != 0) {
                return -6;
            }
        }
    ...
    if (msg->payload_len != 0u) {
        payload_copy = (unsigned char *)malloc((size_t)msg->payload_len);
        if (!payload_copy) {
            return -7;
        }
        memcpy(payload_copy, msg->payload, (size_t)msg->payload_len);
    }

    rc = dg_message_bus_alloc_record_id(bus, &rec_id);
    if (rc != 0) {
...
u32 dg_message_bus_deliver(dg_message_bus *bus, dg_budget *budget, dg_tick current_tick) {
    ...
    for (;;) {
        ...
        r = &bus->records[rec_id];
        dg_message_bus_sub_range(bus, r->hdr.dst_entity, r->hdr.type_id, &sub_start, &sub_count);
...

        {
            ...
            bus->probe_messages_delivered += 1u;
            processed += 1u;
            ...
            if (sub_ofs < sub_count) {
                ...
                if (push_rc != 0) {
                    (void)dg_message_bus_grow_deliver_q(bus, bus->deliver_q.capacity + 1u);
...
static void dg_message_bus_sched_handler(struct dg_sched *sched, void *user_ctx) {
    dg_message_bus *bus = (dg_message_bus *)user_ctx;
    if (!sched || !bus) {
        return;
    }
    (void)dg_message_bus_deliver(bus, &sched->budget, sched->tick);
}
...
int dg_message_bus_install_sense(dg_message_bus *bus, struct dg_sched *sched, u64 priority_key) {
    ...
    return dg_sched_register_phase_handler(sched, DG_PH_SENSE, dg_message_bus_sched_handler, priority_key, bus);
}

u32 dg_message_bus_probe_messages_delivered(const dg_message_bus *bus) {
    return bus ? bus->probe_messages_delivered : 0u;
}
