...
def inspect_save(path):
    ...
    return {
        "kind": "interaction_save",
        "line_count": len(lines),
        "interaction_count": sum(1 for line in lines if line.startswith("interaction_object")),
        "has_interactions": "interactions_begin" in lines and "interactions_end" in lines,
        "sha256": digest(lines),
    }
...
def inspect_replay(path):
    ...
    return {
        ...
        "line_count": len(lines),
        "event_count": sum(1 for line in lines if "client.interaction." in line),
        "sha256": digest(lines),
    }
...
def main():
    parser = argparse.ArgumentParser(description="Inspect deterministic interaction test artifacts.")
