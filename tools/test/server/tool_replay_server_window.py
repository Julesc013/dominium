...
from tools.test.server.server_mvp0_probe import verify_server_window_replay  # noqa: E402


def main() -> int:
    parser = argparse.ArgumentParser(description="Verify SERVER-MVP-0 deterministic tick, loopback, and proof-anchor replay.")
    parser.add_argument("--repo-root", default=REPO_ROOT_HINT)
    parser.add_argument("--output-path", default="")
    args = parser.parse_args()

    repo_root = os.path.normpath(os.path.abspath(str(args.repo_root or REPO_ROOT_HINT)))
    report = verify_server_window_replay(repo_root)
