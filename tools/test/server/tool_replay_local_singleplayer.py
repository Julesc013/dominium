...
from tools.test.server.server_mvp1_probe import verify_local_singleplayer_replay  # noqa: E402


def main() -> int:
    parser = argparse.ArgumentParser(description="Verify SERVER-MVP-1 local singleplayer orchestration replay.")
    parser.add_argument("--repo-root", default=REPO_ROOT_HINT)
    parser.add_argument("--output-path", default="")
    args = parser.parse_args()

    repo_root = os.path.normpath(os.path.abspath(str(args.repo_root or REPO_ROOT_HINT)))
    report = verify_local_singleplayer_replay(repo_root)
