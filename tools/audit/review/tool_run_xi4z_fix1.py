#!/usr/bin/env python3
"""Run deterministic XI-4z approved-lock target normalization."""
...
from tools.audit.review.xi4z_fix1_common import (  # noqa: E402
    Xi4zFix1InputsMissing,
    artifact_hashes,
    build_xi4z_fix1_snapshot,
    write_xi4z_fix1_snapshot,
)


def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Run deterministic XI-4z approved-lock target normalization.")
    parser.add_argument("--repo-root", default=".")
    args = parser.parse_args(argv)

    try:
        first = build_xi4z_fix1_snapshot(args.repo_root)
        second = build_xi4z_fix1_snapshot(args.repo_root)
    except Xi4zFix1InputsMissing as exc:
    ...
    if first_hashes != second_hashes:
        sys.stdout.write(
            json.dumps(
                {
                    "code": "refusal.xi4zfix1.validation_failed",
                    "first_hashes": first_hashes,
                    "reason": "nondeterministic_fix1_outputs",
    ...
    write_xi4z_fix1_snapshot(args.repo_root, first)
    payload = {
