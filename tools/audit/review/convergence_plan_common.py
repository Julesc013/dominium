...
def _required_test_commands(risk_level: str, reasons: Sequence[object], include_convergence_tests: bool = True) -> list[str]:
    ...
    if "protocol_negotiation" in reason_tokens or "semantic_contracts" in reason_tokens:
        commands.append("python tools/package/compatibility/tool_run_interop_stress.py --repo-root .")
    if "pack_compat_install_resolver" in reason_tokens:
        commands.append("python tools/release/mvp/tool_run_product_boot_matrix.py --repo-root .")
        commands.append("python tools/release/mvp/tool_run_all_stress.py --repo-root .")
    if "trust_enforcement" in reason_tokens:
        commands.append("python tools/validators/security/model/tool_run_trust_strict_suite.py --repo-root .")
    if "time_anchor" in reason_tokens:
        commands.append("python tools/domain/time/tool_verify_longrun_ticks.py --repo-root .")
    ...
    if risk_level == "HIGH":
        commands.append("python tools/migration/convergence/tool_run_convergence_gate.py --repo-root .")
    seen: set[str] = set()
    ordered: list[str] = []
...
def _phase_plan(actions: Sequence[Mapping[str, object]]) -> list[dict[str, object]]:
    phase_commands = {
        ...
        "phase_3_high_risk_merges": [
            "python tools/migration/convergence/tool_run_convergence_gate.py --repo-root .",
        ],
        "phase_4_rewire_sweep": [
            "python tools/audit/review/tool_run_duplicate_impl_scan.py --repo-root .",
            "python tools/audit/review/tool_run_implementation_scoring.py --repo-root .",
            "python tools/validators/suite/tool_run_validation.py --repo-root . --profile STRICT",
        ],
        "phase_5_deprecation_and_quarantine": [
            "python tools/audit/review/tool_run_convergence_plan.py --repo-root .",
            "python tools/xstack/testx/runner.py --repo-root . --profile FAST --cache off --subset test_convergence_plan_deterministic,test_decision_rules_stable",
        ],
