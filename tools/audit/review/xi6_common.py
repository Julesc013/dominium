...
from tools.audit.review.architecture_graph_bootstrap_common import (  # noqa: E402
...
def collect_gate_runs(repo_root: str) -> list[dict[str, object]]:
    ...
    return [
        ...
        _run_subprocess_gate(repo_root, "validate_strict", ["python", "-B", "tools/validators/ci/validate_all.py", "--repo-root", ".", "--strict"]),
        _run_subprocess_gate(repo_root, "arch_audit_2", ["python", "-B", "tools/audit/tool_run_arch_audit.py", "--repo-root", "."]),
        _run_subprocess_gate(repo_root, "omega_1_worldgen_lock", ["python", "-B", "tools/domain/worldgen/tool_verify_worldgen_lock.py", "--repo-root", "."]),
        _run_subprocess_gate(repo_root, "omega_2_baseline_universe", ["python", "-B", "tools/release/mvp/tool_verify_baseline_universe.py", "--repo-root", "."]),
        _run_subprocess_gate(repo_root, "omega_3_gameplay_loop", ["python", "-B", "tools/release/mvp/tool_verify_gameplay_loop.py", "--repo-root", "."]),
        _run_subprocess_gate(repo_root, "omega_4_disaster_suite", ["python", "-B", "tools/release/mvp/tool_run_disaster_suite.py", "--repo-root", "."]),
        _run_subprocess_gate(repo_root, "omega_5_ecosystem_verify", ["python", "-B", "tools/release/mvp/tool_verify_ecosystem.py", "--repo-root", "."]),
        _run_subprocess_gate(repo_root, "omega_6_update_sim", ["python", "-B", "tools/release/mvp/tool_run_update_sim.py", "--repo-root", "."]),
        _run_subprocess_gate(repo_root, "trust_strict_suite", ["python", "-B", "tools/validators/security/model/tool_run_trust_strict_suite.py", "--repo-root", "."]),
