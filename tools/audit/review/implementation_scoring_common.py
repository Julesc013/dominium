...
NON_RUNTIME_TOOL_PATH_PREFIXES = ("tools/xstack/auditx/", "tools/audit/review/", "tools/xstack/testx/tests/")
...
def _module_relevance_rank(module_id: object) -> int:
    ...
    if token.startswith(("engine.", "game.", "client.", "server.", "apps.", "launcher.", "setup.")):
        return 3
    if token.startswith("tools.") and not token.startswith(("tools.xstack.auditx.", "tools.audit.review", "tools.xstack.testx.tests")):
        return 2
    if token.startswith(("tests.", "game.tests.", "docs.", "tools.xstack.auditx.", "tools.audit.review", "tools.xstack.testx.tests")):
        return 0
    return 1
