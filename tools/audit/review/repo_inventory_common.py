...
PRODUCT_HINTS = {
    "client": ("tools/release/mvp/runtime_entry.py",),
    "engine": ("tools/validators/shell/product_stub_cli.py",),
    "game": ("tools/validators/shell/product_stub_cli.py",),
    "launcher": ("tools/package/launcher/launch.py",),
    "server": ("apps/server/server_main.py", "tools/release/mvp/runtime_entry.py"),
    "setup": ("tools/package/setup/setup_cli.py",),
...
def _classify_product(rel_path: str) -> str:
    rel_norm = _norm(rel_path)
    if rel_norm.startswith("tools/package/launcher/"):
        return "launcher"
    if rel_norm.startswith("tools/package/setup/"):
        return "setup"
...
def _classify_responsibility(rel_path: str, layer: str) -> str:
    ...
    if rel_norm.startswith(("src/client/render/",)):
        return "ui.rendered"
    if rel_norm.startswith(("src/client/ui/", "apps/workbench/module/tooling/host/win32/", "tools/ui_")):
        return "ui.shared"
...
def _build_target(rel_path: str, product: str, text: str, entrypoints: list[str]) -> str:
    ...
    if rel_norm == "apps/server/server_main.py":
        return "server"
    if rel_norm == "tools/package/launcher/launch.py":
        return "launcher"
    if rel_norm == "tools/package/setup/setup_cli.py":
        return "setup"
    if rel_norm == "tools/release/mvp/runtime_entry.py":
...
def _renderer_surface(repo_root: str, entries: list[dict], product_rows: Mapping[str, object]) -> dict:
    del product_rows
    backend_map = {
        "null": "runtime/render/providers/null/null_renderer.py",
        "software": "runtime/render/providers/software/software_renderer.py",
...
def _scan_product_main_bypasses(repo_root: str, python_files: Iterable[str]) -> list[dict]:
    ...
    for rel_path in sorted(set(str(path) for path in python_files)):
        rel_norm = _norm(rel_path)
        if rel_norm.startswith("tools/") and not rel_norm.startswith(("tools/package/launcher/", "tools/package/setup/", "tools/release/mvp/", "tools/validators/shell/")):
            continue
        if rel_norm not in {
...
def _render_tree(repo_root: str, planned_paths: Iterable[str]) -> str:
    ...
    lines = [
        ...
        "",
        "Status: `DERIVED`",
        "Source: `tools/audit/review/tool_repo_inventory.py`",
        "",
    ]
    for root in TREE_ROOTS:
        rel_root = _norm(root)
