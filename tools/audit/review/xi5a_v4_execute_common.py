...
from tools.audit.review.xi4z_fix1_common import _has_source_like_segment  # noqa: E402
from tools.audit.review.xi4z_fix3_common import (  # noqa: E402
...
def _required_input_paths() -> tuple[str, ...]:
    return (
        ...
        XI_4Z_FIX_FINAL_REL,
        DEPRECATIONS_REL,
        "archive/generated/audit/validation_report_FAST.json",
...
def _ensure_preflight_green(repo_root: str) -> dict[str, str]:
    for profile in ("FAST", "STRICT"):
        subprocess.run(
            ["python", "tools/validators/suite/tool_run_validation.py", "--repo-root", ".", "--profile", profile],
            cwd=repo_root,
    ...
    fast = _load_validation_report(repo_root, "archive/generated/audit/validation_report_FAST.json")
    strict = _load_validation_report(repo_root, "archive/generated/audit/validation_report_STRICT.json")
...
def _gate_plan() -> list[tuple[str, list[str]]]:
    return [
        ...
        ("validate_fast", ["python", "tools/validators/suite/tool_run_validation.py", "--repo-root", ".", "--profile", "FAST"]),
        ("validate_strict", ["python", "tools/validators/suite/tool_run_validation.py", "--repo-root", ".", "--profile", "STRICT"]),
        ("arch_audit_2", ["python", "tools/audit/tool_run_arch_audit.py", "--repo-root", "."]),
        ("omega_1_worldgen_lock", ["python", "tools/domain/worldgen/tool_verify_worldgen_lock.py", "--repo-root", "."]),
        ("omega_2_baseline_universe", ["python", "tools/release/mvp/tool_verify_baseline_universe.py", "--repo-root", "."]),
        ("omega_3_gameplay_loop", ["python", "tools/release/mvp/tool_verify_gameplay_loop.py", "--repo-root", "."]),
        ("omega_4_disaster_suite", ["python", "tools/release/mvp/tool_run_disaster_suite.py", "--repo-root", "."]),
        ("omega_5_ecosystem_verify", ["python", "tools/release/mvp/tool_verify_ecosystem.py", "--repo-root", "."]),
        ("omega_6_update_sim", ["python", "tools/release/mvp/tool_run_update_sim.py", "--repo-root", "."]),
        ("trust_strict_suite", ["python", "tools/validators/security/model/tool_run_trust_strict_suite.py", "--repo-root", "."]),
    ]
