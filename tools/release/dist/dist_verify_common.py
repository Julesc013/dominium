...
REQUIRED_LAYOUT = (
    ...
    "store/contracts/package/locks/pack_lock.mvp_default.json",
    "store/content/profiles/bundles/bundle.mvp_default.json",
        ...
        for rel_path in list(layout.get("missing_paths") or []):
            errors.append(
                _error(
                    ...
                    "rerun python tools/release/dist/tool_assemble_dist_tree.py --repo-root . --platform-tag {} --channel {}".format(_token(platform_tag) or DEFAULT_PLATFORM, DEFAULT_RELEASE_CHANNEL),
                    RULE_VERIFY,
                )
            )
