...
def _legacy_status(rel_path: str) -> str:
    ...
    if token.startswith(("runtime/ui/control/domui/", "runtime/include/", "runtime/view/ir/")) or token.startswith("apps/workbench/module/ui/preview/"):
...
def _legacy_purpose(rel_path: str) -> str:
    token = _norm(rel_path)
    if token.startswith(("runtime/ui/control/domui/", "runtime/include/", "runtime/view/ir/")):
...
def _surface_row(repo_root: str, *, rel_path: str, surface_kind: str, status: str, purpose: str, product_scope: str) -> dict:
    ...
    payload = {
        ...
        "uses_shared_ui_model": bool(
            ...
            or _norm(rel_path) == "runtime/ui/ui_model.py"
        ),
...
def ui_reconcile_violations(repo_root: str) -> list[dict]:
    ...
    for rel_path, rule_id, message in (
        ("runtime/ui/ui_model.py", "INV-UI-SHARES-UI_MODEL", "shared UI model is required"),
...
def render_ui_reconcile_final(report: Mapping[str, object]) -> str:
    ...
    lines.extend(
        (
            ...
            "## Readiness",
            "",
            "- Shared menu/navigation state is centralized in `runtime/ui/ui_model.py`.",
