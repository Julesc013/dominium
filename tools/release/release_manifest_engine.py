...
from tools.release.build_id_engine import build_build_id_input_payload, build_id_identity_from_input_payload
from tools.release.component_graph_resolver import DEFAULT_COMPONENT_GRAPH_ID, load_default_component_graph
...
def _descriptor_sidecar_path(binary_path: str) -> str:
    directory = os.path.dirname(binary_path)
    basename = os.path.basename(binary_path)
    return os.path.join(directory, "{}.descriptor.json".format(basename))


def _load_descriptor_sidecar(binary_path: str) -> tuple[dict, str]:
    sidecar_path = _descriptor_sidecar_path(binary_path)
    payload, error = _read_json(sidecar_path)
    if error:
        return {}, error
    if not payload:
        return {}, "descriptor sidecar is empty"
    return payload, ""
...
def _run_descriptor(binary_path: str) -> tuple[dict, str]:
    ...
    sidecar_descriptor, sidecar_error = _load_descriptor_sidecar(abs_path)
    if sidecar_descriptor:
        return sidecar_descriptor, ""
    if sidecar_error:
        last_error = "{}; sidecar={}".format(last_error, sidecar_error)
    return {}, last_error
