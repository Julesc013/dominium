#!/usr/bin/env python3
"""Generate deterministic REPO-LAYOUT-0 audit outputs."""
...
from tools.release.virtual_paths_common import build_virtual_paths_report, write_virtual_paths_outputs  # noqa: E402


def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Generate REPO-LAYOUT-0 deterministic audit outputs.")
    parser.add_argument("--repo-root", default=".")
    args = parser.parse_args(argv)

    repo_root = os.path.normpath(os.path.abspath(str(args.repo_root)))
    report = build_virtual_paths_report(repo_root)
    written = write_virtual_paths_outputs(repo_root, report)
    payload = {
