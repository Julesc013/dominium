...
def _integration_checks(repo_root: str) -> list[dict]:
    ...
    for rel_path, token, rule_id, code, message in (
        ("tools/package/setup/setup_cli.py", "handle_trust(", RULE_POLICY, "setup_trust_cli_missing", "setup CLI must expose trust commands"),
        ("tools/package/setup/setup_cli.py", "trust_policy_id", RULE_POLICY, "setup_trust_policy_missing", "setup verification and update flows must accept trust_policy_id"),
