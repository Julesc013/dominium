...
from tools.validators.shell.appshell6_probe import verify_supervisor_replay  # noqa: E402


def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Replay deterministic APPSHELL-6 supervision and verify manifest/log stability.")
    parser.add_argument("--repo-root", default=REPO_ROOT_HINT)
    parser.add_argument("--output-path", default="")
    args = parser.parse_args(argv)

    repo_root = os.path.normpath(os.path.abspath(str(args.repo_root or REPO_ROOT_HINT)))
    report = verify_supervisor_replay(repo_root)
