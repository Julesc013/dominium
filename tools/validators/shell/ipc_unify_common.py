...
from runtime.shell.paths import (  # noqa: E402
    VROOT_IPC,
...
from tools.validators.shell.appshell4_probe import run_ipc_attach_probe, verify_ipc_attach_replay  # noqa: E402
...
CANONICAL_SURFACES = (
    {
        "path": "runtime/shell/ipc/ipc_transport.py",
        "surface": "transport",
    ...
    {
        "path": "runtime/shell/ipc/ipc_endpoint_server.py",
    ...
    {
        "path": "runtime/shell/ipc/ipc_client.py",
    ...
    {
        "path": "runtime/shell/supervisor/supervisor_engine.py",
    ...
    {
        "path": "tools/validators/shell/appshell4_probe.py",
        "surface": "probe",
        "classification": "canonical",
...
ALLOWED_DIRECT_SOCKET_USERS = {
    "runtime/shell/ipc/ipc_transport.py": "canonical_transport",
}
ALLOWED_LOW_LEVEL_PRIMITIVE_USERS = {
    "runtime/shell/ipc/ipc_transport.py": "canonical_transport",
    "runtime/shell/ipc/ipc_endpoint_server.py": "canonical_endpoint_server",
    "runtime/shell/ipc/ipc_client.py": "canonical_client",
    "tools/validators/shell/appshell4_probe.py": "test_only_probe",
}
...
LOW_LEVEL_IMPORT_MARKERS = (
    ...
    "from runtime.shell.ipc import",
    "from runtime.shell.ipc.ipc_transport import",
)
SCAN_ROOTS = ("src", "tools")
PY_EXTENSIONS = (".py",)
IPC_SCAN_IGNORE_PATHS = {"tools/validators/shell/ipc_unify_common.py"}
IPC_SCAN_IGNORE_PREFIXES = ("tools/xstack/auditx/analyzers/",)
...
def _doc_header(*, status: str, title: str, canonical: bool) -> str:
    ...
    if canonical:
        lines.extend(
            [
                "Version: 1.0.0",
                "Compatibility: Bound to `docs/canon/constitution_v1.md`, `docs/canon/glossary_v1.md`, `docs/runtime/shell/IPC_ATTACH_CONSOLES.md`, `docs/compatibility/NEGOTIATION_HANDSHAKES.md`, and `docs/release/FROZEN_INVARIANTS_v0_0_0.md`.",
            ]
        )
    lines.extend(
        [
...
def _scan_low_level_ipc_primitive_users(repo_root: str) -> tuple[list[dict], list[dict]]:
    ...
    for rel_path in _python_files(repo_root):
        ...
        if not hits:
            continue
        if rel_path != "runtime/shell/ipc/ipc_transport.py" and not any(marker in text for marker in LOW_LEVEL_IMPORT_MARKERS):
            continue
...
def _vroot_ipc_probe(repo_root: str) -> dict:
    from runtime.shell.ipc import (  # noqa: E402
            ...
            violations.append(
                {
                    ...
                    "file_path": "tools/validators/shell/appshell4_probe.py",
            ...
            violations.append(
                {
                    ...
                    "file_path": "tools/validators/shell/tool_replay_ipc_attach.py",
            ...
            violations.append(
                {
                    ...
                    "file_path": "runtime/shell/ipc/ipc_transport.py",
            ...
            violations.append(
                {
                    ...
                    "file_path": "runtime/shell/ipc/ipc_endpoint_server.py",
            ...
            violations.append(
                {
                    ...
                    "file_path": "docs/runtime/shell/IPC_DISCOVERY.md",
...
def _render_ipc_discovery(report: Mapping[str, object]) -> str:
    ...
    lines = [
        ...
        "- Address payloads: emitted by `runtime/shell/ipc/ipc_transport.py`",
        "",
        "## Discovery Flow",
        "",
