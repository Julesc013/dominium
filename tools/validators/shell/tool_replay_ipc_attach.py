#!/usr/bin/env python3
"""Replay deterministic APPSHELL-4 IPC attach and verify framing/hash stability."""
...
from tools.validators.shell.appshell4_probe import verify_ipc_attach_replay  # noqa: E402


def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Replay deterministic APPSHELL-4 IPC attach and verify framing/hash stability.")
    parser.add_argument("--repo-root", default=REPO_ROOT_HINT)
    parser.add_argument("--output-path", default="")
    args = parser.parse_args(argv)

    repo_root = os.path.normpath(os.path.abspath(str(args.repo_root or REPO_ROOT_HINT)))
    report = verify_ipc_attach_replay(repo_root)
