/*
FILE: tools/validators/suite/validators_registry.cpp
MODULE: Dominium
...
static void validate_render_features(const ValidationContext& ctx, ValidationReport& report) {
    ...
    std::string features_root = ctx.repo_root + "/runtime/render/features";
...
static void validate_render_dirs(const ValidationContext& ctx, ValidationReport& report) {
    ...
    std::string backends_root = ctx.repo_root + "/runtime/render/backends";
        ...
        for (size_t i = 0u; i < entries.size(); ++i) {
            ...
            if (name == "implicit" || name == "legacy") {
                report.add("GOV-VAL-REND-002", VAL_SEV_ERROR,
                           ...
                           "Use renderer identity folders under runtime/render/backends.");
                continue;
            }
    ...
    for (size_t i = 0u; i < entries.size(); ++i) {
        ...
        if (is_backend_name(entries[i].name)) {
            report.add("GOV-VAL-REND-002", VAL_SEV_ERROR,
                       ...
                       "backend folder outside runtime/render/backends",
                       "Move backend folders under runtime/render/backends only.");
        }
    }
