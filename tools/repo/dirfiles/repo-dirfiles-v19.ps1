...
Default behavior:
  ...
  - [STEP] is cyan, [RUN] and live progress are yellow, [PASS] is green, warnings use orange in truecolor mode and yellow fallback, errors are red.
...
$SchemaName = "repo-dirfiles-json-v19"
...
function Get-AnsiForRole {
    ...
    switch ($Role) {
        ...
        "PROGRESS" { return "$($script:Esc)[1;38;2;255;235;0m" }     # bright yellow live line
        "WARN"     { return "$($script:Esc)[1;38;2;255;165;0m" }     # orange warning; truecolor only
        "NOTE"     { return "$($script:Esc)[1;38;2;255;165;0m" }     # orange note; truecolor only
...
function Get-ConsoleColorForRole {
    ...
    switch ($Role) {
        ...
        "PROGRESS" { return [ConsoleColor]::Yellow }
...
try {
    Write-EventLine -Level "STEP" -Message "V19 startup: yellow RUN/progress and orange WARN truecolor palette enabled."
    ...
    $utf8 = New-Object System.Text.UTF8Encoding -ArgumentList $false
    [System.IO.File]::WriteAllText($script:RunLogPath, "repo-dirfiles v19 run log`n", $utf8)
