...
GUARANTEE_SCHEMA_REL = os.path.join("contracts", "schemas", "meta", "observability_guarantee.schema")
GUARANTEE_SCHEMA_JSON_REL = os.path.join("contracts", "schemas", "observability_guarantee.schema.json")
...
INTEGRATION_TOKENS = (
    ...
    ("tools/package/setup/setup_cli.py", "update.check.result", "setup update check must emit the guaranteed update category", RULE_GUARANTEES),
    ("tools/package/setup/setup_cli.py", "update.plan.generated", "setup update plan must emit the guaranteed update category", RULE_GUARANTEES),
    ("tools/package/setup/setup_cli.py", "update.apply.completed", "setup update apply must emit the guaranteed update category", RULE_GUARANTEES),
    ("tools/package/setup/setup_cli.py", "update.rollback.completed", "setup rollback must emit the guaranteed update category", RULE_GUARANTEES),
    ("tools/package/setup/setup_cli.py", "lib.install.status", "setup install status must emit the guaranteed lib category", RULE_GUARANTEES),
    ("tools/package/setup/setup_cli.py", "lib.install.plan", "setup install planning must emit the guaranteed lib category", RULE_GUARANTEES),
    ("tools/package/setup/setup_cli.py", "lib.install.apply", "setup install apply must emit the guaranteed lib category", RULE_GUARANTEES),
    ("tools/package/setup/setup_cli.py", "lib.save.migrated", "setup save migration must emit the guaranteed lib category", RULE_GUARANTEES),
    ("tools/package/setup/setup_cli.py", "lib.instance.migrated", "setup instance migration must emit the guaranteed lib category", RULE_GUARANTEES),
    ...
    ("tools/package/setup/setup_cli.py", "appshell.refusal", "setup refusal paths must emit the refusal category", RULE_GUARANTEES),
)
...
def observability_violations(repo_root: str) -> list[dict]:
    ...
    for rel_path, message, rule_id in (
        ...
        (os.path.join("tools", "repo", "meta", "audit", "observability_common.py"), "observability helper is required", RULE_GUARANTEES),
        (os.path.join("tools", "repo", "meta", "audit", "tool_run_observability.py"), "observability runner is required", RULE_GUARANTEES),
    ):
