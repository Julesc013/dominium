...
def build_governance_model_report(repo_root: str, *, platform_tag: str = "win64") -> dict:
    ...
    for rel_path, token, code, message in (
        ("tools/package/setup/setup_cli.py", "def handle_governance(", "setup_governance_status_missing", "setup CLI must expose governance status"),
        ("tools/package/setup/setup_cli.py", "trust status", "setup_trust_status_missing", "setup CLI must expose trust status"),
