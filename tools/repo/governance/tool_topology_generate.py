...
CONTROL_PLANE_MODULE_ID = "module:tools/repo/governance/control_plane_engine.py"
CONTROL_SUBSYSTEM_MODULES = (
    ("control_plane_engine", "tools/repo/governance/control_plane_engine.py"),
    ("negotiation_kernel", "tools/repo/governance/negotiation/negotiation_kernel.py"),
    ("control_ir_engine", "tools/repo/governance/ir/control_ir_compiler.py"),
    ("view_engine", "tools/repo/governance/view/view_engine.py"),
    ("fidelity_engine", "tools/repo/governance/fidelity/fidelity_engine.py"),
    ("effect_engine", "tools/repo/governance/effects/effect_engine.py"),
)
...
def generate_topology_map(
    *,
    ...
    if any(node.get("node_id") == "tool:tools/repo/governance/tool_topology_generate.py" for node in tool_nodes):
        edges.append(
            _edge(
                edge_kind="produces",
                from_node_id="tool:tools/repo/governance/tool_topology_generate.py",
                to_node_id="contract_set:governance.topology_map_artifact",
            )
        )
    if any(node.get("node_id") == "tool:tools/repo/governance/tool_semantic_impact.py" for node in tool_nodes):
        edges.append(
            _edge(
                edge_kind="consumes",
                from_node_id="tool:tools/repo/governance/tool_semantic_impact.py",
                to_node_id="contract_set:governance.topology_map_artifact",
            )
        )
    ...
    payload = {
        ...
        "extensions": {
            "generator": "tools/repo/governance/tool_topology_generate.py",
        },
    }
