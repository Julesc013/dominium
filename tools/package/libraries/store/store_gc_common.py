...
from tools.package.libraries.bundle.bundle_manifest import build_bundle_manifest, write_json as write_bundle_json
from tools.package.libraries.install.install_validator import (
...
from tools.package.libraries.save.save_validator import (
    deterministic_fingerprint as save_deterministic_fingerprint,
    write_json as write_save_json,
)
from tools.package.libraries.store import (
...
def build_store_gc_report(repo_root: str) -> dict:
        ...
        violations.append(
            {
                ...
                "file_path": "runtime/storage/gc_engine.py",
            }
        )
    ...
    gc_engine_text = _read_text(root, "runtime/storage/gc_engine.py")
...
def store_gc_violations(repo_root: str) -> list[dict]:
    ...
    required_paths = (
        ...
        ("contracts/abi/store/reachability_engine.py", "reachability engine is required", RULE_GRAPH),
        ("runtime/storage/gc_engine.py", "gc engine is required", RULE_GRAPH),
        ("tools/package/libraries/store/store_gc_common.py", "STORE-GC helper is required", RULE_DETERMINISTIC),
        ("tools/package/libraries/store/tool_store_verify.py", "store verification tool is required", RULE_DETERMINISTIC),
        ("tools/package/libraries/store/tool_run_store_gc.py", "STORE-GC runner is required", RULE_DETERMINISTIC),
    ...
    setup_text = _read_text(root, "tools/package/setup/setup_cli.py")
    if "handle_store(" not in setup_text or "\"store\"" not in setup_text:
        violations.append(
            {"rule_id": RULE_POLICY, "code": "setup_store_gc_missing", "message": "setup CLI must expose `setup store gc`", "file_path": "tools/package/setup/setup_cli.py"}
        )
