...
from tools.package.libraries.stress.lib_stress_common import (
...
def main(argv: list[str] | None = None) -> int:
    ...
    if report_out:
        ...
        canonical_report = os.path.normpath(os.path.abspath(os.path.join(repo_root, DEFAULT_REPORT_REL)))
        write_json(default_report, report)
        if os.path.normcase(default_report) != os.path.normcase(canonical_report):
