...
def build_migration_lifecycle_report(repo_root: str) -> dict:
    ...
    for rel_path, token, rule_id, code, message in (
        ...
        ("tools/package/compatibility/tool_plan_migration.py", "plan_artifact_migration(", RULE_SILENT, "plan_tool_missing_hook", "migration planning tool must route through canonical planning helper"),
        ("tools/package/compatibility/tool_apply_migration.py", "apply_artifact_migration(", RULE_SILENT, "apply_tool_missing_hook", "migration apply tool must route through canonical apply helper"),
        ("tools/package/setup/setup_cli.py", "migrate-save", RULE_SILENT, "setup_migrate_save_missing", "setup CLI must expose migrate-save"),
        ("tools/package/setup/setup_cli.py", "migrate-instance", RULE_SILENT, "setup_migrate_instance_missing", "setup CLI must expose migrate-instance"),
    ):
        text = ""
        ...
        violations.append(
            {
                ...
                "file_path": "tools/validators/compatibility/migration_lifecycle.py",
            }
        )
...
def render_migration_lifecycle_baseline(report: Mapping[str, object]) -> str:
    ...
    lines.extend(
        [
            ...
            "## Readiness",
            "",
            "- migrate vs read-only vs refuse decisions are centralized in `tools/validators/compatibility/migration_lifecycle.py`",
...
def migration_lifecycle_violations(repo_root: str) -> list[dict]:
    ...
    required_paths = (
        ...
        ("tools/package/compatibility/migration_lifecycle_common.py", "migration lifecycle common helper is required", RULE_POLICY),
        ("tools/package/compatibility/tool_plan_migration.py", "migration planning tool is required", RULE_SILENT),
        ("tools/package/compatibility/tool_apply_migration.py", "migration apply tool is required", RULE_SILENT),
