...
Modes:
  default       Auto-best mode: Git tracked scan when inside a Git worktree; otherwise filesystem scan.
  -TrackedOnly Force Git-index scan using git ls-files.
  -Filesystem  Force filesystem scan.
  default zip   Auto-best mode: use 7z/7za/7zr for ZIP Deflate when available; otherwise .NET ZIP.
  -No7Zip      Force .NET ZIP even if 7-Zip is available.
#>
...
param(
    ...
    # Controls console noise. Compact is the right-click default; Detailed/Trace are intentionally chatty.
    [ValidateSet("Live", "Dashboard", "Silent", "Compact", "Detailed", "Trace")]
    ...
    # Default is readable ANSI color when supported. Use -Theme Mono for plain text or -Theme Color for legacy Write-Host colors.
    [ValidateSet("Rich", "Mono", "Color")]
    [string]$Theme = "Rich",
...
$SchemaName          = "repo-dirfiles-json-v10"
$ManifestSchemaName  = "repo-dirfiles-manifest-v10"
$RunLogFileName      = "dirfiles_run.log"

$LeanExcludeDirNames = @(
    "node_modules", ".venv", "venv", "env", "__pycache__",
    ".pytest_cache", ".mypy_cache", ".ruff_cache", ".tox", ".nox",
    ".next", ".nuxt", ".gradle", ".idea", ".vs", ".cache",
    "bin", "obj", "build", "dist", "target", "coverage",
    "out", "_deps", "CMakeFiles", "cmake-build-debug", "cmake-build-release"
)

$script:StartTime = Get-Date
$script:ExitCode = 0
$script:UseColor = (($Theme -ne "Mono") -and (-not $NoColor))
$script:UseAnsi = $false
$script:UseHostColor = $script:UseColor
$script:Esc = [char]27
$script:Warnings = New-Object System.Collections.ArrayList
$script:WarningCount = 0
if ($Trace) { $StatusMode = "Trace" }
$script:StatusMode = $StatusMode
$script:RunLogPath = $null
$script:PendingLogLines = New-Object System.Collections.ArrayList
$script:Events = New-Object System.Collections.ArrayList
$script:DashboardAvailable = $false
$script:DashboardActive = $false
$script:DashboardInitialized = $false
$script:DashboardLastRender = [datetime]"2000-01-01T00:00:00"
$script:DashboardStatus = "Starting"
$script:DashboardLevel = "INFO"
$script:DashboardTarget = ""
$script:DashboardOutput = ""
$script:DashboardSourceMode = ""
$script:DashboardZipMode = "auto"
$script:DashboardZipEngine = "pending"
$script:CurrentPath = ""
$script:CurrentStats = $null
$script:StageCurrent = $null
$script:StageTotal = $null
$script:LiveLineActive = $false
$script:LiveLastRender = [datetime]"2000-01-01T00:00:00"
$script:LiveLastLength = 0
if ($script:StatusMode -eq "Dashboard") { $script:StatusMode = "Live" }
if ($script:StatusMode -eq "Live") { $ProgressPreference = "SilentlyContinue" }
if ($script:StatusMode -eq "Dashboard") { $ProgressPreference = "SilentlyContinue" }
...
function Enable-VirtualTerminal {
    if (-not $script:UseColor) { return $false }
    ...
    if ([System.Environment]::OSVersion.Platform -eq [System.PlatformID]::Win32NT) {
        try {
            $type = "RepoDirfilesConsoleNative" -as [type]
            if ($null -eq $type) {
                $source = @"
using System;
...
"@
            ...
            $type = "RepoDirfilesConsoleNative" -as [type]
            if ($null -ne $type) {
                $handle = [RepoDirfilesConsoleNative]::GetStdHandle(-11)
                [int]$mode = 0
                $ok = [RepoDirfilesConsoleNative]::GetConsoleMode($handle, [ref]$mode)
                if ($ok) {
                    $newMode = ($mode -bor 4)
                    [void][RepoDirfilesConsoleNative]::SetConsoleMode($handle, $newMode)
                    return $true
                }
            }
        }
...
function Initialize-ConsoleTheme {
    ...
    if ($Theme -eq "Rich") {
        $script:UseAnsi = Enable-VirtualTerminal
        $script:UseHostColor = (-not $script:UseAnsi)
        return
    }

    if ($Theme -eq "Color") {
        $script:UseAnsi = $false
        $script:UseHostColor = $true
        return
    }
...
function Get-StyleCode {
    param([string]$Style)

    switch ($Style) {
        "Title"     { return "1;97" }
        "Subtitle"  { return "3;90" }
        "Border"    { return "90" }
        "Label"     { return "1;96" }
        "Value"     { return "97" }
        "Path"      { return "37" }
...
function Style-Text {
    ...
    $code = Get-StyleCode -Style $Style
    $esc = [string]$script:Esc
...
function Write-DashboardLine {
    ...
    $line = Fit-Line -Text $Text -Width $Width
    if ($script:UseAnsi) {
        Write-Host (Style-Text -Text $line -Style $Style)
    }
    elseif ($script:UseHostColor) {
        $color = "Gray"
        if ($Style -eq "Title") { $color = "White" }
        elseif ($Style -eq "Border") { $color = "DarkGray" }
        elseif ($Style -eq "Good") { $color = "Green" }
        elseif ($Style -eq "Warn") { $color = "Yellow" }
        elseif ($Style -eq "Error") { $color = "Red" }
        elseif ($Style -eq "Step") { $color = "Cyan" }
...
function Write-DashboardKV {
    ...
    $prefix = ("{0,-8}: " -f $Label)
    $maxValue = $Width - $prefix.Length - 1
    if ($maxValue -lt 8) { $maxValue = 8 }
    $short = Limit-TextMiddle -Text $Value -MaxLength $maxValue
    $visible = $prefix + $short
    $padLen = ($Width - 1) - $visible.Length
    if ($padLen -lt 0) { $padLen = 0 }
    $pad = " " * $padLen

    if ($script:UseAnsi) {
        Write-Host ((Style-Text -Text $prefix -Style "Label") + (Style-Text -Text $short -Style $ValueStyle) + $pad)
    }
    elseif ($script:UseHostColor) {
        Write-Host (Fit-Line -Text $visible -Width $Width) -ForegroundColor "Gray"
    }
    else {
        Write-Host (Fit-Line -Text $visible -Width $Width)
    }
}

function Write-DashboardStatusLine {
    param(
        [string]$Level,
        [string]$Message,
        [int]$Width
    ...
    $maxMsg = $Width - $prefix.Length - $levelText.Length - 1
    if ($maxMsg -lt 8) { $maxMsg = 8 }
    $short = Limit-TextMiddle -Text $Message -MaxLength $maxMsg
    $visible = $prefix + $levelText + $short
    $padLen = ($Width - 1) - $visible.Length
    if ($padLen -lt 0) { $padLen = 0 }
    $pad = " " * $padLen

    if ($script:UseAnsi) {
        $style = Get-LevelStyle -Level $Level
        Write-Host ((Style-Text -Text $prefix -Style "Label") + (Style-Text -Text $levelText -Style $style) + (Style-Text -Text $short -Style "Value") + $pad)
    }
    elseif ($script:UseHostColor) {
        Write-Host (Fit-Line -Text $visible -Width $Width) -ForegroundColor (Get-LevelColor -Level $Level)
    }
    else {
        Write-Host (Fit-Line -Text $visible -Width $Width)
    }
...
function Write-DashboardProgressLine {
    param(
        [int]$Current,
        [int]$Total,
        [int]$Width
    ...
    $barWidth = [Math]::Min(34, [Math]::Max(12, $Width - 58))
    if ($Total -le 0) {
        Write-DashboardKV -Label "Progress" -Value "Reports [stage active]" -Width $Width -ValueStyle "Accent"
        return
    }

    if ($Current -lt 0) { $Current = 0 }
    if ($Current -gt $Total) { $Current = $Total }
    $filled = [int][Math]::Floor(($Current / [double]$Total) * $barWidth)
    ...
    $visible = $prefix + "[" + $filledText + $emptyText + $suffix
    $padLen = ($Width - 1) - $visible.Length
    if ($padLen -lt 0) { $padLen = 0 }
    $pad = " " * $padLen

    if ($script:UseAnsi) {
        Write-Host ((Style-Text -Text "Progress: " -Style "Label") + (Style-Text -Text "Reports " -Style "Value") + "[" + (Style-Text -Text $filledText -Style "BarFill") + (Style-Text -Text $emptyText -Style "BarEmpty") + $suffix + $pad)
    }
    else {
        Write-Host (Fit-Line -Text $visible -Width $Width)
    }
...
function Write-DashboardEventLine {
    param(
        [object]$Event,
        [int]$Width
    )

    $stamp = [string]$Event.stamp
    ...
    $maxMsg = $Width - $prefix.Length - $levelText.Length - 1
    if ($maxMsg -lt 8) { $maxMsg = 8 }
    $short = Limit-TextMiddle -Text $msg -MaxLength $maxMsg
    $visible = $prefix + $levelText + $short
    $padLen = ($Width - 1) - $visible.Length
    if ($padLen -lt 0) { $padLen = 0 }
    $pad = " " * $padLen
    ...
    if ($script:UseAnsi) {
        $style = Get-LevelStyle -Level $level
        Write-Host ((Style-Text -Text $prefix -Style "Dim") + (Style-Text -Text $levelText -Style $style) + (Style-Text -Text $short -Style "Value") + $pad)
    }
    elseif ($script:UseHostColor) {
        Write-Host (Fit-Line -Text $visible -Width $Width) -ForegroundColor (Get-LevelColor -Level $level)
    }
...
function Get-ConsoleWidthSafe {
    try {
        ...
        try {
            $w2 = [int]$Host.UI.RawUI.WindowSize.Width
            if ($w2 -lt 60) { return 80 }
            return $w2
        }
        catch { return 80 }
    }
...
function Limit-TextMiddle {
    ...
    if ($MaxLength -lt 16) { return $Text }
    if ($Text.Length -le $MaxLength) { return $Text }

    $left = [Math]::Max(6, [int](($MaxLength - 5) / 2))
    $right = [Math]::Max(6, $MaxLength - $left - 5)
...
function Clear-LiveLine {
    if (-not $script:LiveLineActive) { return }
    try {
...
function Format-LiveStatusText {
    param(
        [string]$Label,
        [hashtable]$Stats,
        [string]$CurrentPath
    ...
    $nodes = 0; $dirs = 0; $files = 0; $missing = 0; $errors = 0
    if ($null -ne $Stats) {
        if ($Stats.ContainsKey("nodes")) { $nodes = $Stats["nodes"] }
        if ($Stats.ContainsKey("directories")) { $dirs = $Stats["directories"] }
        if ($Stats.ContainsKey("files")) { $files = $Stats["files"] }
        if ($Stats.ContainsKey("missing")) { $missing = $Stats["missing"] }
        if ($Stats.ContainsKey("read_errors")) { $errors = $Stats["read_errors"] }
    }

    $stage = ""
    if ($null -ne $script:StageCurrent -and $null -ne $script:StageTotal -and [int]$script:StageTotal -gt 0) {
        $bar = New-Bar -Current ([int]$script:StageCurrent) -Total ([int]$script:StageTotal) -Width 12
        $stage = " rep=$($script:StageCurrent)/$($script:StageTotal) $bar"
    }

    if ([string]::IsNullOrWhiteSpace($CurrentPath)) { $CurrentPath = $script:CurrentPath }
    if ([string]::IsNullOrWhiteSpace($Label)) { $Label = $script:DashboardStatus }

    $plain = ("{0} | {1}{2} | n={3} d={4} f={5} m={6} e={7} w={8} | {9}" -f `
        $elapsedText,
        $Label,
        $stage,
        $nodes,
...
function Write-LiveStatus {
    ...
    if ($script:StatusMode -ne "Live") { return }

    $now = Get-Date
    if (-not $Force) {
        $ms = ($now - [datetime]$script:LiveLastRender).TotalMilliseconds
        if ($ms -lt $DashboardRefreshMs) { return }
    }

    $script:LiveLastRender = $now

    $width = Get-ConsoleWidthSafe
    if ($width -lt 60) { $width = 80 }

    $plain = Format-LiveStatusText -Label $Label -Stats $Stats -CurrentPath $CurrentPath
    $plain = Limit-TextMiddle -Text $plain -MaxLength ($width - 1)
    $plain = Fit-Line -Text $plain -Width $width

    $styled = $plain
    if ($script:UseAnsi) {
        $styled = Style-Text -Text $plain -Style "Accent"
    }

    Write-Host ("`r" + $styled) -NoNewline
    $script:LiveLineActive = $true
    $script:LiveLastLength = $plain.Length
}

function Test-DashboardAvailable {
    if ($script:StatusMode -ne "Dashboard") { return $false }

    try {
        if ([Console]::IsOutputRedirected) { return $false }
        $w = [int][Console]::WindowWidth
        $h = [int][Console]::WindowHeight
        if ($w -lt 60 -or $h -lt 15) { return $false }
        return $true
    }
    catch {
        return $false
...
function Initialize-Dashboard {
    ...
    $script:DashboardAvailable = Test-DashboardAvailable
    $script:DashboardActive = $script:DashboardAvailable

    if ($script:DashboardActive) {
        try {
...
function Add-DashboardEvent {
    ...
    while ($script:Events.Count -gt 50) {
...
function Write-RunLogLine {
    ...
    try {
        [System.IO.File]::AppendAllText($script:RunLogPath, $line + [Environment]::NewLine, (New-Utf8NoBomEncoding))
    }
    catch {
        # Do not fail the main run because diagnostic logging failed.
    }
}

function Set-RunLogPath {
    param([string]$OutputDir)

    try {
        $script:RunLogPath = Join-Path $OutputDir $RunLogFileName
        $header = "repo-dirfiles run log UTC=" + (Get-Date).ToUniversalTime().ToString("o") + [Environment]::NewLine
        [System.IO.File]::WriteAllText($script:RunLogPath, $header, (New-Utf8NoBomEncoding))

        foreach ($line in @($script:PendingLogLines.ToArray())) {
            [System.IO.File]::AppendAllText($script:RunLogPath, ([string]$line) + [Environment]::NewLine, (New-Utf8NoBomEncoding))
        }
...
function Set-DashboardContext {
    ...
    if (-not [string]::IsNullOrWhiteSpace($TargetRoot)) { $script:DashboardTarget = $TargetRoot }
    if (-not [string]::IsNullOrWhiteSpace($OutputDir)) { $script:DashboardOutput = $OutputDir }
    if (-not [string]::IsNullOrWhiteSpace($SourceMode)) { $script:DashboardSourceMode = $SourceMode }
    if (-not [string]::IsNullOrWhiteSpace($ZipMode)) { $script:DashboardZipMode = $ZipMode }
    if (-not [string]::IsNullOrWhiteSpace($ZipEngine)) { $script:DashboardZipEngine = $ZipEngine }
}

function Render-Dashboard {
    param([bool]$Force = $false)

    if ($script:StatusMode -ne "Dashboard") { return }
    Initialize-Dashboard
    if (-not $script:DashboardActive) { return }

    $now = Get-Date
    if (-not $Force) {
        $ms = ($now - [datetime]$script:DashboardLastRender).TotalMilliseconds
        if ($ms -lt $DashboardRefreshMs) { return }
    }

    try {
        $width = Get-ConsoleWidthSafe
        if ($width -lt 72) { $width = 72 }
        $elapsed = $now - $script:StartTime
        $elapsedText = "{0:hh\:mm\:ss}" -f $elapsed
        $border = "+" + ("-" * ($width - 2)) + "+"

        $stats = $script:CurrentStats
        $nodes = 0; $dirs = 0; $files = 0; $missing = 0; $errors = 0
        if ($null -ne $stats) {
            if ($stats.ContainsKey("nodes")) { $nodes = $stats["nodes"] }
            if ($stats.ContainsKey("directories")) { $dirs = $stats["directories"] }
        ...
        $shortTarget = Limit-TextMiddle -Text $script:DashboardTarget -MaxLength ($width - 20)
        $shortOutput = Limit-TextMiddle -Text $script:DashboardOutput -MaxLength ($width - 20)
        $shortCurrent = Limit-TextMiddle -Text $script:CurrentPath -MaxLength ($width - 20)
        $mode = "source=" + $script:DashboardSourceMode + "  zip=" + $script:DashboardZipMode + "  engine=" + $script:DashboardZipEngine + "  ui=" + $script:StatusMode + "  theme=" + $Theme
        $counts = "nodes=" + $nodes + "  dirs=" + $dirs + "  files=" + $files + "  missing=" + $missing + "  read_errors=" + $errors + "  warnings=" + $script:WarningCount
        $logHint = "tmp/" + $RunLogFileName + "  |  tmp/" + $ManifestFileName

        [Console]::SetCursorPosition(0, 0)
        [Console]::Clear()

        Write-DashboardLine -Text $border -Width $width -Style "Border"
        $title = "| repo-dirfiles v10    repository map generator    elapsed " + $elapsedText
        Write-DashboardLine -Text $title -Width $width -Style "Title"
        Write-DashboardLine -Text $border -Width $width -Style "Border"
        Write-DashboardStatusLine -Level $script:DashboardLevel -Message $script:DashboardStatus -Width $width
        Write-DashboardKV -Label "Target" -Value $shortTarget -Width $width -ValueStyle "Path"
        Write-DashboardKV -Label "Output" -Value $shortOutput -Width $width -ValueStyle "Path"
        Write-DashboardKV -Label "Mode" -Value $mode -Width $width -ValueStyle "Dim"
        Write-DashboardKV -Label "Counts" -Value $counts -Width $width -ValueStyle "Value"
        Write-DashboardKV -Label "Current" -Value $shortCurrent -Width $width -ValueStyle "Dim"

        if ($null -ne $script:StageCurrent -and $null -ne $script:StageTotal) {
            Write-DashboardProgressLine -Current ([int]$script:StageCurrent) -Total ([int]$script:StageTotal) -Width $width
        }
        else {
            Write-DashboardProgressLine -Current 0 -Total 0 -Width $width
        }

        Write-DashboardKV -Label "Log" -Value $logHint -Width $width -ValueStyle "Accent"
        Write-DashboardLine -Text $border -Width $width -Style "Border"
        Write-DashboardLine -Text "Recent events:" -Width $width -Style "Subtitle"

        $events = @($script:Events.ToArray())
        $n = [Math]::Min($DashboardEventLines, $events.Count)
        $start = $events.Count - $n
        if ($start -lt 0) { $start = 0 }
        for ($i = $start; $i -lt $events.Count; $i++) {
            Write-DashboardEventLine -Event $events[$i] -Width $width
        }

        $used = 13 + $n
        while ($used -lt ([Math]::Min([Console]::WindowHeight - 1, 24))) {
            Write-DashboardLine -Text "" -Width $width -Style "Value"
            $used++
        }

        $script:DashboardLastRender = $now
    }
    catch {
        $script:DashboardActive = $false
        $script:StatusMode = "Compact"
        try { [Console]::CursorVisible = $true } catch { }
    }
}

function Write-PlainLogLine {
    param(
        [string]$Level,
        [string]$Message
    )
    ...
    if ($script:StatusMode -eq "Silent") {
        if ($Level -ne "PASS" -and $Level -ne "WARN" -and $Level -ne "ERROR") { return }
    }

    if ($Level -eq "TRACE" -and $script:StatusMode -ne "Trace") { return }
    ...
    if ($script:UseAnsi) {
        $style = Get-LevelStyle -Level $Level
        Write-Host ((Style-Text -Text ("[" + $stamp + "] ") -Style "Dim") + (Style-Text -Text ("[" + $Level.PadRight(5) + "] ") -Style $style) + (Style-Text -Text $Message -Style "Value"))
    }
    elseif ($script:UseHostColor) {
...
function Write-Log {
    ...
    Write-RunLogLine -Level $Level -Message $Message
    Add-DashboardEvent -Level $Level -Message $Message

    $script:DashboardStatus = $Message
    $script:DashboardLevel = $Level

    if ($Message -match '\[(\d+)\/(\d+)\]') {
        $script:StageCurrent = [int]$matches[1]
        $script:StageTotal = [int]$matches[2]
    }
    elseif ($Level -eq "STEP" -and $Message -notmatch 'Generating \[') {
        $script:StageCurrent = $null
        $script:StageTotal = $null
    }

    if ($script:StatusMode -eq "Dashboard") {
        $force = ($Level -eq "ERROR" -or $Level -eq "WARN" -or $Level -eq "PASS" -or $Level -eq "STEP")
        Render-Dashboard -Force:$force
    }
    elseif ($script:StatusMode -eq "Live") {
        Write-PlainLogLine -Level $Level -Message $Message
        Write-LiveStatus -Label $Message -Stats $script:CurrentStats -CurrentPath $script:CurrentPath -Force:$true
    }
    else {
        Write-PlainLogLine -Level $Level -Message $Message
    }
...
function Write-ScanUpdate {
    ...
    $script:CurrentStats = $Stats
    $script:CurrentPath = $CurrentPath
    $basic = "nodes=$($Stats['nodes']) dirs=$($Stats['directories']) files=$($Stats['files'])"

    if ($script:StatusMode -eq "Dashboard") {
        $script:DashboardStatus = $Label
        Render-Dashboard -Force:$false
        return
    }

    if ($script:StatusMode -eq "Live") {
        Write-LiveStatus -Label $Label -Stats $Stats -CurrentPath $CurrentPath -Force:$false
        return
    }

    if (-not $NoProgress) {
        Write-Progress -Activity $Label -Status $basic
    }

    if ($script:StatusMode -eq "Detailed") {
        $width = Get-ConsoleWidthSafe
        $pathMax = [Math]::Max(24, $width - 70)
        $shortPath = Limit-TextMiddle -Text $CurrentPath -MaxLength $pathMax
        Write-Log -Message ("scan $basic path=$shortPath")
    }
    elseif ($script:StatusMode -eq "Trace") {
        Write-Log -Level "TRACE" -Message ("scan $basic path=$CurrentPath")
    }
}
    ...
    while ($stack.Count -gt 0) {
        ...
        if ([string]$entry["kind"] -eq "dir") {
            ...
            if (($stats["nodes"] % $StatusEvery) -eq 0 -or ((Get-Date) - $lastLog).TotalSeconds -ge $HeartbeatSeconds) {
                Write-ScanUpdate -Label "Scanning filesystem report" -Stats $stats -CurrentPath $repoPath
                $lastLog = Get-Date
            }
...
function Build-TrackedIndex {
    ...
    if ($null -eq $lines) { throw "Git tracked scan was requested, but 'git ls-files' failed. Run inside a Git worktree or force -Filesystem." }

    if ($IncludeUntracked) {
        $untracked = Invoke-GitLines -DirectoryPath $RootPath -Arguments @("-c", "core.quotepath=false", "ls-files", "--others", "--exclude-standard")
        if ($null -ne $untracked) { $lines = @($lines) + @($untracked) }
    }
...
function Build-TrackedReportNodes {
    ...
    while ($stack.Count -gt 0) {
        ...
        if ([string]$entry["kind"] -eq "dir") {
            ...
            if (($stats["nodes"] % $StatusEvery) -eq 0 -or ((Get-Date) - $lastLog).TotalSeconds -ge $HeartbeatSeconds) {
                Write-ScanUpdate -Label "Scanning tracked report" -Stats $stats -CurrentPath $repoPath
                $lastLog = Get-Date
            }
...
function New-ZipFromFiles {
    ...
    try {
        ...
        $engine = "dotnet-zip"
        $trySevenZip = (-not $No7Zip)
        if ($trySevenZip) {
            $seven = Get-7ZipCommand
            if ([string]::IsNullOrWhiteSpace($seven)) {
                if ($Use7Zip) { Add-WarningMessage -Message "-Use7Zip was supplied, but 7z/7za/7zr was not found. Falling back to .NET ZIP." }
            }
            else {
...
try {
    Write-Log -Level "STEP" -Message "V10 startup: live status UI enabled; best defaults selected."
    ...
    if ($TrackedOnly -and $Filesystem) { throw "Use only one of -TrackedOnly or -Filesystem." }

    $sourceMode = "filesystem"
    $sourceModeReason = "forced filesystem fallback"
    if ($TrackedOnly) {
        $sourceMode = "git_tracked"
        $sourceModeReason = "forced by -TrackedOnly"
    }
    elseif ($Filesystem) {
        $sourceMode = "filesystem"
        $sourceModeReason = "forced by -Filesystem"
    }
    elseif ($gitMetadata["is_git_worktree"] -eq $true) {
        $sourceMode = "git_tracked"
        $sourceModeReason = "auto-selected because target is a Git worktree"
    }
    else {
        $sourceMode = "filesystem"
        $sourceModeReason = "auto-selected because no Git worktree was detected"
    }

    $zipModeLabel = "auto-7z"
    if ($No7Zip) { $zipModeLabel = "dotnet" }
    if ($NoZip) { $zipModeLabel = "disabled" }
    Set-DashboardContext -TargetRoot $targetRootPath -OutputDir $outputDir -SourceMode $sourceMode -ZipMode $zipModeLabel -ZipEngine "pending"
    ...
    Write-Log -Message "Source mode:            $sourceMode ($sourceModeReason)"
    Write-Log -Message "Console status mode:    $script:StatusMode"
    Write-Log -Message "Schema:                 $SchemaName"
    Write-Log -Message "Lean mode:              $Lean"
    Write-Log -Message "ZIP mode:               auto 7z unless unavailable or -No7Zip"
    ...
    Set-RunLogPath -OutputDir $outputDir
    Set-DashboardContext -TargetRoot $targetRootPath -OutputDir $outputDir -SourceMode $sourceMode -ZipMode $zipModeLabel -ZipEngine "pending"
    ...
    if ($NoZip) {
        ...
        Set-DashboardContext -ZipEngine $zipEngine
    ...
    $manifest = [ordered]@{
        ...
        _meta = [ordered]@{
            ...
            source_mode = $sourceMode
            source_mode_reason = $sourceModeReason
            status_mode = $script:StatusMode
            theme = $Theme
            run_log_file = $RunLogFileName
