...
from tools.domain.embodiment.earth6_probe import verify_collision_window_replay  # noqa: E402


def main() -> int:
    parser = argparse.ArgumentParser(description="Verify EARTH-6 terrain collision replay determinism.")
    parser.add_argument("--repo-root", default=REPO_ROOT_HINT)
    parser.add_argument("--output-path", default="")
    args = parser.parse_args()

    repo_root = os.path.normpath(os.path.abspath(str(args.repo_root or REPO_ROOT_HINT)))
    report = verify_collision_window_replay(repo_root)
