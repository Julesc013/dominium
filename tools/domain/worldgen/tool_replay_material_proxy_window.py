#!/usr/bin/env python3
"""Verify EARTH-10 material-proxy replay determinism."""
...
from tools.domain.worldgen.earth10_probe import verify_material_proxy_window_replay  # noqa: E402


def main() -> int:
    parser = argparse.ArgumentParser(description="Verify EARTH-10 material-proxy replay determinism.")
    parser.add_argument("--repo-root", default=REPO_ROOT_HINT)
    parser.add_argument("--output-path", default="")
    args = parser.parse_args()

    repo_root = os.path.normpath(os.path.abspath(str(args.repo_root or REPO_ROOT_HINT)))
    report = verify_material_proxy_window_replay(repo_root)
