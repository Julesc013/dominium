"""E457 unnamed-RNG smell analyzer."""
...
ANALYZER_ID = "E457_UNNAMED_RNG_SMELL"
...
def run(graph, repo_root, changed_files=None):
    ...
    for row in list(report.get("blocking_findings") or []):
        finding = dict(row or {})
        if str(finding.get("category", "")).strip() != "determinism.unnamed_rng":
            continue
        rel_path = str(finding.get("path", "")).replace("\\", "/")
        message = str(finding.get("message", "")).strip() or "unnamed RNG detected in a truth path"
        evidence = [message]
        ...
        findings.append(
            make_finding(
                ...
                category="arch.unnamed_rng_smell",
                severity="RISK",
                confidence=0.98,
                file_path=rel_path,
