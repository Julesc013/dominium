"""E203 unbudgeted thermal solve smell analyzer."""
...
ANALYZER_ID = "E203_UNBUDGETED_THERM_SOLVE_SMELL"


class UnbudgetedThermSolveSmell:
    analyzer_id = ANALYZER_ID


_SOLVE_CALL_PATTERN = re.compile(r"\bsolve_thermal_network_t1\s*\(", re.IGNORECASE)
...
def run(graph, repo_root, changed_files=None):
    ...
    for token in required_tokens:
        ...
        findings.append(
            make_finding(
                analyzer_id=ANALYZER_ID,
                category="architecture.unbudgeted_therm_solve_smell",
                severity="VIOLATION",
                confidence=0.95,
                file_path=stress_tool_rel,
                line=1,
                evidence=["missing thermal budget/degradation token", token],
                suggested_classification="INVALID",
                recommended_action="REWRITE",
                related_invariants=["INV-THERM-BUDGETED"],
                ...
                for line_no, line in enumerate(text.splitlines(), start=1):
                    ...
                    findings.append(
                        make_finding(
                            ...
                            file_path=rel_path,
                            line=line_no,
                            evidence=["thermal solve call outside canonical budgeted tool/engine path", snippet[:140]],
