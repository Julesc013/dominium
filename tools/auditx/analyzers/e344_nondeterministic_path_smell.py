...
REQUIRED_FILES = {
    "game/domains/geology/path/path_engine.py": (
        "geo_path_query(",
    ...
    "game/domains/geology/path/shard_route_planner.py": (
    ...
    for rel_path, required_tokens in sorted(REQUIRED_FILES.items()):
        ...
        findings.append(
            make_finding(
                ...
                related_invariants=["INV-PATHING-DETERMINISTIC"],
                related_paths=[rel_path, "game/domains/geology/path/path_engine.py"],
            )
        )
    return findings
