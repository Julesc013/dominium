...
def run(graph, repo_root, changed_files=None):
    ...
    for rel_path in SCAN_FILES:
        ...
        if not text:
            continue
        if rel_path == "runtime/appshell/logging/log_engine.py" and "redact_observability_mapping(" not in text:
            findings.append(
                make_finding(
            ...
            continue
        if rel_path != "runtime/appshell/logging/log_engine.py" and SECRET_RE.search(text) and "appshell.refusal" not in text:
            findings.append(
                make_finding(
                    analyzer_id=ANALYZER_ID,
                    category="observability.secrets_in_log_smell",
                    ...
                    related_invariants=[RULE_ID],
                    related_paths=[rel_path, "runtime/appshell/logging/log_engine.py"],
                )
            )
    return findings
