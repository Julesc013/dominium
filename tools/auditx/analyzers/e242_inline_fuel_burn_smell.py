"""E242 inline fuel-burn smell analyzer."""
...
ANALYZER_ID = "E242_INLINE_FUEL_BURN_SMELL"


class InlineFuelBurnSmell:
    analyzer_id = ANALYZER_ID


_INLINE_FUEL_PATTERNS = (
            ...
            for name in files:
                ...
                for line_no, line in enumerate(text.splitlines(), start=1):
                    ...
                    if (not snippet) or snippet.startswith("#"):
                        continue
                    if not any(pattern.search(snippet) for pattern in _INLINE_FUEL_PATTERNS):
                        continue
                    findings.append(
                        make_finding(
                            analyzer_id=ANALYZER_ID,
                            category="architecture.inline_fuel_burn_smell",
                            severity="RISK",
                            confidence=0.85,
                            file_path=rel_path,
                            line=line_no,
                            evidence=["inline fuel decrement detected outside canonical combustion handlers", snippet[:140]],
