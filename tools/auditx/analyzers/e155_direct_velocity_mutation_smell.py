"""E155 direct velocity mutation smell analyzer."""
...
ANALYZER_ID = "E155_DIRECT_VELOCITY_MUTATION_SMELL"


class DirectVelocityMutationSmell:
    analyzer_id = ANALYZER_ID


_DIRECT_MUTATION_PATTERNS = (
    re.compile(r"\bvelocity_mm_per_tick\b\s*=", re.IGNORECASE),
    re.compile(r"\baccel_mm_per_tick2\b\s*=", re.IGNORECASE),
            ...
            for name in files:
                ...
                for line_no, line in enumerate(text.splitlines(), start=1):
                    ...
                    findings.append(
                        make_finding(
                            analyzer_id=ANALYZER_ID,
                            category="architecture.direct_velocity_mutation_smell",
                            severity="RISK",
                            confidence=0.9,
                            file_path=rel_path,
                            line=line_no,
                            evidence=["direct velocity/accel mutation outside mobility process/solver path", snippet[:140]],
