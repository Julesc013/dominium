"""E550 missing migration policy smell analyzer."""
...
ANALYZER_ID = "E550_MISSING_MIGRATION_POLICY_SMELL"
TARGET_CATEGORIES = {"ecosystem_verify.migration_missing"}
    ...
    for row in list(dict(report or {}).get("blocking_findings") or []):
        ...
        findings.append(
            make_finding(
                ...
                category="ecosystem_verify.missing_migration_policy_smell",
                severity="RISK",
                confidence=0.99,
                file_path=rel_path,
                line=int(finding.get("line", 1) or 1),
                evidence=[
                    str(finding.get("message", "")).strip() or "ecosystem migration coverage failed",
                    str(finding.get("snippet", "")).strip()[:160],
                ],
                suggested_classification="INVALID",
                recommended_action="DECLARE_OR_RESTORE_MIGRATION_POLICY_COVERAGE_FOR_ALL_GOVERNED_ARTIFACT_KINDS",
