"""STRICT test: per-tick and checkpoint hash anchors replay identically."""
...
TEST_ID = "testx.srz.hash_anchor_replay"
...
def run(repo_root: str):
    ...
    created = _create_session(repo_root, ".srz.hash_replay")
    if created.get("result") != "complete":
        return {"status": "fail", "message": "session create failed before SRZ hash replay test"}
    ...
    if first.get("result") != "complete" or second.get("result") != "complete":
        return {"status": "fail", "message": "script replay failed before SRZ hash comparison"}
    ...
    for key in keys:
        if str(first.get(key, "")) != str(second.get(key, "")):
            return {"status": "fail", "message": "SRZ replay mismatch for '{}'".format(key)}

    if list(first.get("tick_hash_anchors") or []) != list(second.get("tick_hash_anchors") or []):
        return {"status": "fail", "message": "per-tick SRZ hash anchors changed across identical replay"}
    if list(first.get("checkpoint_hashes") or []) != list(second.get("checkpoint_hashes") or []):
        return {"status": "fail", "message": "checkpoint hashes changed across identical replay"}
    return {"status": "pass", "message": "SRZ per-tick/checkpoint replay hash equivalence passed"}
