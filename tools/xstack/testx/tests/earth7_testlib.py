...
def wind_field_report(repo_root: str) -> dict:
    ...
    from tools.domain.worldgen.earth7_probe import verify_wind_window_replay as _report
...
def latitude_band_report(repo_root: str) -> dict:
    _ensure_repo_root(repo_root)
    from tools.domain.worldgen.earth7_probe import wind_latitude_band_report as _report
...
def seasonal_shift_report(repo_root: str) -> dict:
    _ensure_repo_root(repo_root)
    from tools.domain.worldgen.earth7_probe import wind_seasonal_shift_report as _report
...
def poll_advection_report(repo_root: str) -> dict:
    _ensure_repo_root(repo_root)
    from tools.domain.worldgen.earth7_probe import poll_advection_bias_report as _report
...
def earth_wind_hash(repo_root: str) -> str:
    _ensure_repo_root(repo_root)
    from tools.domain.worldgen.earth7_probe import run_wind_tick_fixture
