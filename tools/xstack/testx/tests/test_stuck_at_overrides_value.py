"""STRICT test: LOGIC-8 stuck-at faults override sampled values deterministically."""
...
TEST_ID = "test_stuck_at_overrides_value"
...
def run(repo_root: str):
    ...
    from game.domain.logic.eval import process_logic_network_evaluate
    from game.domain.logic.fault import process_logic_fault_set
    from game.domain.logic.signal import process_signal_emit_pulse, process_signal_set
    ...
    for port_id, value in (("in.a", 0), ("in.b", 1)):
        seeded = process_signal_set(
            current_tick=0,
            signal_store_state=signal_store_state,
            signal_request={
                "network_id": "net.logic.fault.stuck",
    ...
    if str(fault_result.get("result", "")) != "complete":
        return {"status": "fail", "message": "logic fault set refused valid stuck-at fixture"}
    ...
    result = process_logic_network_evaluate(
        ...
        evaluation_request={"network_id": "net.logic.fault.stuck"},
    ...
    if str(result.get("result", "")) != "complete":
        return {"status": "fail", "message": "logic evaluation did not complete with a valid stuck-at fixture"}
    snapshot = dict(result.get("sense_snapshot") or {})
    and_inputs = dict(dict(snapshot.get("inputs_by_element") or {}).get("inst.logic.and.1") or {})
    if int(dict(and_inputs.get("in.a") or {}).get("value", 0)) != 1:
        return {"status": "fail", "message": "stuck-at-1 fault did not override the targeted sampled value"}
    explain_rows = [dict(row) for row in list(result.get("explain_artifact_rows") or []) if isinstance(row, dict)]
    if not any(str(dict(row.get("extensions") or {}).get("event_kind_id", "")).strip() == "explain.logic_stuck_at" for row in explain_rows):
        return {"status": "fail", "message": "stuck-at fault did not produce explain.logic_stuck_at"}
    return {"status": "pass", "message": "stuck-at faults deterministically override targeted sampled values"}
