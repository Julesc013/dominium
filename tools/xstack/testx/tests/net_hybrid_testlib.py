"""Shared helpers for deterministic MP-5 SRZ hybrid tests."""
...
def prepare_hybrid_runtime_fixture(
    repo_root: str,
    *,
    save_id: str,
    client_peer_id: str = "peer.client.hybrid.alpha",
) -> Dict[str, object]:
    if repo_root not in sys.path:
        sys.path.insert(0, repo_root)

    from src.net.policies.policy_srz_hybrid import initialize_hybrid_runtime
    ...
    created = create_session_spec(
        ...
        rng_seed_string="seed.mp5.hybrid.fixture",
        rng_roots=[],
        universe_identity_path="",
        universe_seed_string="seed.mp5.hybrid.universe",
        universe_id="",
        entitlements=[
            "session.boot",
            "entitlement.inspect",
        ...
        net_endpoint="loopback://mp5.hybrid.fixture",
        net_transport_id="transport.loopback",
        net_client_peer_id=str(client_peer_id),
        net_server_peer_id="peer.server.hybrid",
    ...
    initialized = initialize_hybrid_runtime(
        ...
        server_policy_registry=dict(payloads.get("net_server_policy_registry_hash") or {}),
    ...
    if str(initialized.get("result", "")) != "complete":
        ...
        raise RuntimeError("initialize_hybrid_runtime refused ({})".format(reason))
