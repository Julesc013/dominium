"""STRICT test: multiplayer pipeline executes net baseline/join stages for server-authoritative policy."""
...
TEST_ID = "testx.net.pipeline_net_handshake_stage_authoritative"
...
def run(repo_root: str):
    ...
    created = create_session_spec(
        repo_root=repo_root,
        save_id="save.testx.net.pipeline.authoritative",
        ...
        rng_seed_string="seed.mp4.pipeline.authoritative",
        rng_roots=[],
        universe_identity_path="",
        universe_seed_string="seed.mp4.pipeline.authoritative.universe",
        universe_id="",
        entitlements=[
            "session.boot",
            "entitlement.inspect",
        ...
        net_endpoint="loopback://mp4.pipeline.authoritative",
        net_transport_id="transport.loopback",
        net_client_peer_id="peer.client.pipeline.auth",
        net_server_peer_id="peer.server.pipeline.auth",
    ...
    if str(created.get("result", "")) != "complete":
        return {"status": "fail", "message": "create_session_spec failed for authoritative pipeline stage test"}
    ...
    if str(sync.get("policy_id", "")) != "policy.net.server_authoritative":
        return {"status": "fail", "message": "unexpected baseline policy id in run summary"}
    if not str(join.get("snapshot_id", "")).strip():
        return {"status": "fail", "message": "join summary missing snapshot_id"}

    return {"status": "pass", "message": "server-authoritative net stage chain executed deterministically"}
