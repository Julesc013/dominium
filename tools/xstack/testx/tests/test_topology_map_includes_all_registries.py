"""FAST test: topology map includes all registry files."""
...
TEST_ID = "test_topology_map_includes_all_registries"
TEST_TAGS = ["fast", "governance", "topology", "registry"]
...
def run(repo_root: str):
    ...
    declared = sorted(
        set(
            ...
            if isinstance(row, dict) and str(row.get("node_kind", "")).strip() == "registry"
        )
    )
    expected = _registry_paths(repo_root)
    missing = sorted(token for token in expected if token not in set(declared))
    if missing:
        return {
            "status": "fail",
            "message": "topology map missing registry nodes (count={}): {}".format(len(missing), ", ".join(missing[:8])),
        }
    return {"status": "pass", "message": "topology map registry coverage passed"}
