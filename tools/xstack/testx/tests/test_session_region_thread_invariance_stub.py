"""STRICT test: region-management traversal remains deterministic across worker-count stub settings."""
...
TEST_ID = "testx.session.region_thread_invariance_stub"
...
def run(repo_root: str):
    ...
    save_id = str(session_fixture.get("save_id", "save.testx.session.fixture")) + ".region.thread_stub"
    ...
    if created.get("result") != "complete":
        return {"status": "fail", "message": "session create failed before region thread invariance test"}
    ...
    if single_worker.get("result") != "complete" or dual_worker.get("result") != "complete":
        return {"status": "fail", "message": "region traversal failed for worker-count invariance check"}
    if str(single_worker.get("final_state_hash", "")) != str(dual_worker.get("final_state_hash", "")):
        return {"status": "fail", "message": "final_state_hash changed across worker-count stub settings"}
    if list(single_worker.get("state_hash_anchors") or []) != list(dual_worker.get("state_hash_anchors") or []):
        return {"status": "fail", "message": "state hash anchors changed across worker-count stub settings"}
    if str(single_worker.get("deterministic_fields_hash", "")) != str(dual_worker.get("deterministic_fields_hash", "")):
        return {"status": "fail", "message": "deterministic fields hash changed across worker-count stub settings"}
    if int(dual_worker.get("workers_effective", 0)) != 1:
        return {"status": "fail", "message": "worker-count stub must report deterministic single-worker scheduler"}
    return {"status": "pass", "message": "region thread-count invariance stub check passed"}
