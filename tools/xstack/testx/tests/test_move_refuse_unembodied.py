"""STRICT test: process.agent_move refuses unembodied agents deterministically."""
...
TEST_ID = "testx.embodiment.move_refuse_unembodied"
        ...
        "controller_assemblies": [
            {
                ...
                "binding_ids": ["binding.control.possess.unembodied"],
                "status": "active",
            }
        ],
        "control_bindings": [
            {
                "binding_id": "binding.control.possess.unembodied",
                "controller_id": "controller.alpha",
...
def _run_once():
    ...
    result = execute_intent(
        state=state,
        intent={
            "intent_id": "intent.agent.move.unembodied.001",
...
def run(repo_root: str):
    ...
    if str(first.get("result", "")) != "refused" or str(second.get("result", "")) != "refused":
        return {"status": "fail", "message": "unembodied movement should deterministically refuse"}
    refusal_first = dict(first.get("refusal") or {})
    refusal_second = dict(second.get("refusal") or {})
    if str(refusal_first.get("reason_code", "")) != "refusal.agent.unembodied":
        return {"status": "fail", "message": "expected refusal.agent.unembodied for unembodied move"}
    if refusal_first != refusal_second:
        return {"status": "fail", "message": "unembodied refusal payload must be deterministic"}
    return {"status": "pass", "message": "unembodied movement refusal determinism check passed"}
