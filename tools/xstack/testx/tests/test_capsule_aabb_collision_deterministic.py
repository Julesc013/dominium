"""STRICT test: capsule-aabb collision resolution is deterministic."""
...
TEST_ID = "testx.embodiment.capsule_aabb_collision_deterministic"
...
def _state() -> dict:
    return {
        ...
        "body_assemblies": [
            {
                "assembly_id": "body.dynamic.capsule",
            ...
            {
                "assembly_id": "body.static.box",
                "owner_assembly_id": "object.earth",
                "shape_type": "aabb",
                "shape_parameters": {
                    "radius_mm": 0,
                    "height_mm": 0,
                    "half_extents_mm": {"x": 1200, "y": 1200, "z": 1200},
                    "vertex_ref_id": "",
                },
                "collision_layer": "layer.default",
                "dynamic": False,
...
def _run_once():
    ...
    result = execute_intent(
        state=state,
        intent={
            "intent_id": "intent.body_move.capsule_aabb.001",
            "process_id": "process.body_move_attempt",
            "inputs": {
                "body_id": "body.dynamic.capsule",
                "delta_transform_mm": {"x": 2200, "y": 0, "z": 0},
                "dt_ticks": 1,
            },
        },
...
def run(repo_root: str):
    ...
    if str(first_result.get("result", "")) != "complete" or str(second_result.get("result", "")) != "complete":
        return {"status": "fail", "message": "capsule-aabb move attempt must complete deterministically"}
    ...
    if canonical_sha256(first_state) != canonical_sha256(second_state):
        return {"status": "fail", "message": "capsule-aabb final state hash diverged across identical runs"}
    if str(first_result.get("state_hash_anchor", "")) != str(second_result.get("state_hash_anchor", "")):
        return {"status": "fail", "message": "state hash anchor diverged for capsule-aabb deterministic run"}

    first_collision = dict(first_state.get("collision_state") or {})
    if "body.dynamic.capsule::body.static.box" not in list(first_collision.get("last_tick_resolved_pairs") or []):
        return {"status": "fail", "message": "expected resolved pair body.dynamic.capsule::body.static.box was not recorded"}
    if list(first_collision.get("last_tick_unresolved_pairs") or []):
        return {"status": "fail", "message": "capsule-aabb test should not leave unresolved overlap pairs"}
    return {"status": "pass", "message": "capsule-aabb collision determinism check passed"}
