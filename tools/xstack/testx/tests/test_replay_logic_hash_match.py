"""STRICT test: LOGIC-4 replay tool produces stable hash summaries."""
...
TEST_ID = "test_replay_logic_hash_match"
TEST_TAGS = ["strict", "logic", "eval", "replay", "proof"]
...
def run(repo_root: str):
    ...
    _, logic_network_state = make_chain_network(network_id="net.logic.eval.replay")
    state = None
    for port_id in ("in.a", "in.b"):
        seeded = process_signal_set(
            ...
            signal_request={
                "network_id": "net.logic.eval.replay",
    ...
    scenario = {
        ...
        "evaluation_requests": [
            {"tick": 1, "network_id": "net.logic.eval.replay"},
            {"tick": 2, "network_id": "net.logic.eval.replay"},
            {"tick": 3, "network_id": "net.logic.eval.replay"},
        ],
    }
    first = replay_logic_window_from_payload(repo_root=repo_root, payload=scenario)
    second = replay_logic_window_from_payload(repo_root=repo_root, payload=scenario)
    if str(first.get("result", "")) != "complete" or str(second.get("result", "")) != "complete":
        return {"status": "fail", "message": "logic replay tool did not complete"}
    for key in (
        ...
        if str(first.get(key, "")) != str(second.get(key, "")):
            return {"status": "fail", "message": "logic replay hash '{}' drifted across equivalent runs".format(key)}
    return {"status": "pass", "message": "logic replay hash summary is stable"}
