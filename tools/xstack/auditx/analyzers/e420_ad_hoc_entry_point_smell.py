...
REQUIRED_TOKENS = {
    ...
    "archive/generated/dist/bin/dominium_client": (
        "client_main(sys.argv[1:])",
    ),
    "archive/generated/dist/bin/dominium_server": (
...
def run(graph, repo_root, changed_files=None):
    ...
    for rel_path, tokens in REQUIRED_TOKENS.items():
        ...
        if missing:
            ...
            continue
        if rel_path.startswith("archive/generated/dist/bin/") and "tool_emit_descriptor" in text:
            findings.append(
                make_finding(
                    analyzer_id=ANALYZER_ID,
                    category="appshell.ad_hoc_entry_point_smell",
