"""E338 raw sqrt usage smell analyzer."""
...
WATCH_PREFIXES = ("game/domain/mobility/", "game/domain/fields/", "game/domain/pollution/", "game/domain/systems/")
RAW_SQRT_TOKENS = ("math.sqrt(", "math.isqrt(")
ALLOWLIST = {
    "game/domain/geology/kernel/geo_kernel.py",
    ...
    for rel_path in sorted(set(candidates)):
        ...
        for line_no, line in enumerate(text.splitlines(), start=1):
            ...
            if not snippet or snippet.startswith("#"):
                continue
            if not any(token in snippet for token in RAW_SQRT_TOKENS):
                continue
            findings.append(
                make_finding(
                    analyzer_id=ANALYZER_ID,
                    category="geometry.raw_sqrt_usage_smell",
                    severity="RISK",
                    confidence=0.96,
                    file_path=rel_path,
                    line=line_no,
                    evidence=[
                        "raw sqrt/isqrt distance math detected outside GEO metric engine",
                        snippet[:160],
                    ],
