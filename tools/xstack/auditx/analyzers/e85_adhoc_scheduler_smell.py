"""E85 ad-hoc scheduler smell analyzer."""
...
ANALYZER_ID = "E85_ADHOC_SCHEDULER_SMELL"
...
def run(graph, repo_root, changed_files=None):
    ...
    core_path = "tools/validators/core/schedule/schedule_engine.py"
    core_text = _read_text(repo_root, core_path)
    if not core_text:
        findings.append(
            make_finding(
                analyzer_id=ANALYZER_ID,
                category="architecture.adhoc_scheduler_smell",
                severity="VIOLATION",
                confidence=0.95,
                file_path=core_path,
                line=1,
    ...
    for root, _dirs, files in os.walk(src_root):
        for name in files:
            ...
            if rel_path.startswith("src/core/schedule/"):
                continue
