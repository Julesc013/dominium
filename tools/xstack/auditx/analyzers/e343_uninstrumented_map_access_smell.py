...
REQUIRED_FILES = {
    "game/domain/geology/lens/lens_engine.py": (
        "build_lens_request(",
        "build_projected_view_artifact(",
        "map_instrument_required",
        "allow_omniscient_debug",
    ),
    "game/domain/geology/lens/cctv_engine.py": (
    ...
    for rel_path, required_tokens in sorted(REQUIRED_FILES.items()):
        ...
        findings.append(
            make_finding(
                ...
                related_invariants=["INV-VIEWS-MUST-USE-LENS", "INV-OMNISCIENCE-PROFILED"],
                related_paths=[rel_path, "game/domain/geology/projection/projection_engine.py"],
            )
        )
    return findings
