...
def _collect_declared_capabilities(repo_root, file_nodes):
    ...
    for node in file_nodes:
        rel = node.label
        if not rel.startswith(("data/", "contracts/schema/", "docs/")):
            continue
...
def _collect_capability_references(repo_root, file_nodes):
    ...
    for node in file_nodes:
        rel = node.label
        if rel.startswith(("data/", "contracts/schema/", "docs/")):
            continue
...
def run(graph, repo_root, changed_files=None):
    ...
    for node in file_nodes:
        rel = node.label
        if rel.startswith(("legacy/", "docs/", "tests/", "archive/generated/dist/", "out/", "build/")):
            continue
        if not rel.lower().endswith(SOURCE_EXTS):
            continue
        text = _read_text(os.path.join(repo_root, rel.replace("/", os.sep)))
