    ...
    for row in list(report.get("blocking_findings") or []):
        ...
        findings.append(
            make_finding(
                ...
                severity="BLOCKER",
                confidence=0.99,
                file_path=rel_path or "tools/release/dist/dist_tree_common.py",
                evidence=[
                ...
                related_paths=[rel_path or "tools/release/dist/dist_tree_common.py", "tools/release/component_graph_resolver.py"],
            )
        )
    return findings
