...
REQUIRED_FILES = {
    "game/domain/geology/worldgen/worldgen_engine.py": (
    ...
    "game/domain/geology/projection/projection_engine.py": (
        "build_worldgen_requests_for_projection(",
    ),
    "game/domain/systems/roi/system_roi_scheduler.py": (
    ...
    for rel_path, required_tokens in sorted(REQUIRED_FILES.items()):
        ...
        findings.append(
            make_finding(
                ...
                related_paths=[rel_path, "game/domain/geology/worldgen/worldgen_engine.py", "tools/xstack/sessionx/process_runtime.py"],
            )
        )
    return findings
