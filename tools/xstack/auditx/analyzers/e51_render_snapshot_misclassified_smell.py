...
DOC_PATH = "docs/runtime/render/RENDER_SNAPSHOT_ARTIFACTS.md"
PIPELINE_PATHS = (
    ...
    if not derived_anchor:
        findings.append(
            make_finding(
                ...
                severity="RISK",
                confidence=0.86,
                file_path="runtime/render/backend/snapshot_capture.py",
                line=1,
                evidence=["No deterministic derived storage anchor found (run_meta/render_snapshots)."],
                suggested_classification="INVALID",
                recommended_action="ADD_RULE",
                related_invariants=["INV-RENDER_SNAPSHOTS_DERIVED_ONLY"],
                related_paths=["runtime/render/backend/snapshot_capture.py", "tools/validators/render/tool_render_capture.py"],
            )
        )
