...
def run(graph, repo_root, changed_files=None):
    ...
    allowed_files = {
        ...
        "game/domain/interior/compartment_flow_engine.py",
        "game/domain/interior/compartment_flow_builder.py",
        ...
        for walk_root, _dirs, files in os.walk(root):
            for name in files:
                ...
                if rel_path in allowed_files:
                    continue
                if not (rel_path.startswith("game/domain/fluids/") or rel_path.startswith("game/domain/interior/") or rel_path.startswith("tools/xstack/sessionx/")):
                    continue
                ...
                for line_no, line in enumerate(text.splitlines(), start=1):
                    ...
                    findings.append(
                        make_finding(
                            ...
                            related_paths=[rel_path, "game/domain/fluids/network/fluid_network_engine.py", "game/domain/interior/compartment_flow_engine.py"],
                        )
                    )
                    break
    return sorted(
