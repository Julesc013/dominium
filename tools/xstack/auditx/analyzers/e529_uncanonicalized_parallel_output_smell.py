    ...
    for row in list(dict(payload or {}).get("blocking_findings") or []):
        ...
        findings.append(
            make_finding(
                ...
                severity="RISK",
                confidence=0.99,
                file_path=rel_path or "runtime/shell/supervisor/supervisor_engine.py",
                evidence=[
                ...
                related_paths=[rel_path or "runtime/shell/supervisor/supervisor_engine.py", "engine/concurrency/canonical_merge.py"],
            )
        )
    return findings
