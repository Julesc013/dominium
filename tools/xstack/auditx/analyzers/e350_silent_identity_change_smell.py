...
WATCH_PREFIXES = ("game/domain/geology/", "tools/validators/domain/geology/", "tools/xstack/sessionx/")
REQUIRED_FILES = {
    "game/domain/geology/overlay/overlay_merge_engine.py": (
...
_SKIP_PREFIXES = ("tools/xstack/testx/tests/", "tools/xstack/auditx/analyzers/", "docs/")
    ...
    for rel_path, required_tokens in sorted(REQUIRED_FILES.items()):
        ...
        findings.append(
            make_finding(
                ...
                related_invariants=["INV-NO-IDENTITY-OVERRIDE-WITHOUT-MIGRATION"],
                related_paths=[rel_path, "game/domain/geology/overlay/overlay_merge_engine.py"],
            )
        )
