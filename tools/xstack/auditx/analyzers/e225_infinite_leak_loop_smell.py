...
def run(graph, repo_root, changed_files=None):
    ...
    for walk_root, _dirs, files in os.walk(scan_root):
        for name in files:
            ...
            if rel_path.startswith("tools/xstack/testx/tests/") or rel_path.startswith("tools/xstack/auditx/analyzers/"):
                continue
            ...
            for line_no, line in enumerate(rows, start=1):
                ...
                findings.append(
                    make_finding(
                        ...
                        related_paths=[rel_path, "game/domain/fluids/network/fluid_network_engine.py"],
                    )
                )
                break
    return sorted(
