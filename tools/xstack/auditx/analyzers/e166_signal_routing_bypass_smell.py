...
def run(graph, repo_root, changed_files=None):
    ...
    transport_rel = "game/domain/signals/transport/transport_engine.py"
    ...
    if transport_text and "query_route_result(" not in transport_text:
        findings.append(
            make_finding(
                ...
                related_paths=[transport_rel, "tools/validators/core/graph/routing_engine.py"],
            )
        )

    allow_files = {
        "game/domain/signals/transport/transport_engine.py",
                ...
                for line_no, line in enumerate(text.splitlines(), start=1):
                    ...
                    findings.append(
                        make_finding(
                            ...
                            recommended_action="REWRITE",
                            related_invariants=["INV-SIGNALS-USE-ABS-ROUTING"],
                            related_paths=[rel_path, "tools/validators/core/graph/routing_engine.py"],
                        )
                    )
                    break
    return sorted(
