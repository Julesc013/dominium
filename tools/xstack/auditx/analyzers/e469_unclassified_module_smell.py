...
from tools.audit.review.repo_inventory_common import SCAN_ROOTS, load_or_run_inventory_report, unknown_inventory_entries
    ...
    if str(dict(report or {}).get("inventory_id", "")).strip() != "repo.inventory.v1":
        findings.append(
            make_finding(
                ...
                severity="RISK",
                confidence=0.94,
                file_path="archive/generated/audit/repo_inventory.json",
                evidence=["repository inventory report is missing or invalid"],
                suggested_classification="TODO-BLOCKED",
                recommended_action="ADD_RULE",
                related_invariants=[RULE_ID],
                related_paths=["archive/generated/audit/repo_inventory.json"],
            )
        )
        return findings
