"""E34 Order bypass smell analyzer."""
...
ANALYZER_ID = "E34_ORDER_BYPASS_SMELL"
    ...
    for root in roots:
        ...
        for walk_root, _dirs, files in os.walk(abs_root):
            for name in files:
                ...
                if rel_path.startswith(("tools/xstack/testx/tests/", "tools/xstack/auditx/")):
                    continue
                out.append(rel_path)
...
def run(graph, repo_root, changed_files=None):
    ...
    if not os.path.isfile(target_abs):
        findings.append(
            make_finding(
                ...
                category="civilisation.order_bypass_smell",
                severity="RISK",
                confidence=0.95,
                file_path=target_rel,
                line=1,
                evidence=["process runtime missing; order intent enforcement cannot be verified."],
                suggested_classification="TODO-BLOCKED",
                recommended_action="ADD_RULE",
                related_invariants=["INV-ORDERS-AS-INTENTS"],
    ...
    for token in REQUIRED_TOKENS:
        ...
        findings.append(
            make_finding(
                ...
                file_path=target_rel,
                line=1,
                evidence=["Missing order processing token.", token],
    ...
    bypass_pattern = re.compile(r"\b(execute_order|order_executor)\s*\(", re.IGNORECASE)
    for rel_path in _scan_targets(repo_root):
        if rel_path == target_rel:
            continue
        for line_no, line in _iter_lines(repo_root, rel_path):
            if not bypass_pattern.search(str(line)):
                continue
            findings.append(
                make_finding(
                    analyzer_id=ANALYZER_ID,
                    ...
                    line=line_no,
                    evidence=[
                        "Suspicious direct order execution API usage detected.",
                        str(line).strip()[:140],
                    ],
                    suggested_classification="INVALID",
