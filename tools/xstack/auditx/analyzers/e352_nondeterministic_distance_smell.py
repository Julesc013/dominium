"""E352 nondeterministic distance smell analyzer."""

from __future__ import annotations

import os

from analyzers.base import make_finding


ANALYZER_ID = "E352_NONDETERMINISTIC_DISTANCE_SMELL"
REQUIRED_FILES = {
    "game/domain/geology/metric/metric_engine.py": (
        "geo_distance(",
        "geo_geodesic(",
        "metric_query_proof_surface(",
        "metric_cache_lookup(",
        "metric_cache_store(",
    ),
    "game/domain/geology/metric/metric_cache.py": (
        "metric_cache_enabled(",
        "metric_cache_key(",
        "_evict(",
        "sorted(",
        "reference_mode",
    ),
    "tools/validators/domain/geology/tool_run_geo_stress.py": (
        "verify_geo_stress_scenario(",
        "cross_platform_determinism_hash",
        "stable_across_repeated_runs",
    ),
    "tools/validators/domain/geology/tool_replay_geo_window.py": (
        "distance_query_fingerprint",
        "metric_query_hash_chain",
        "stable_across_repeated_runs",
    ),
}
WATCH_PREFIXES = ("game/domain/geology/metric/", "tools/validators/domain/geology/")
_BANNED_TOKENS = ("random.", "uuid", "secrets.", "time.time(", "datetime.now(", "os.urandom(")


def _read_text(repo_root: str, rel_path: str) -> str:
    abs_path = os.path.join(repo_root, rel_path.replace("/", os.sep))
    try:
        return open(abs_path, "r", encoding="utf-8", errors="ignore").read()
    except OSError:
        return ""


def _iter_candidate_files(repo_root: str):
    for prefix in WATCH_PREFIXES:
        abs_prefix = os.path.join(repo_root, prefix.replace("/", os.sep))
        if not os.path.isdir(abs_prefix):
            continue
        for root, _dirs, files in os.walk(abs_prefix):
            for name in sorted(files):
                if not name.endswith(".py"):
                    continue
                abs_path = os.path.join(root, name)
                yield os.path.relpath(abs_path, repo_root).replace(os.sep, "/")


def run(graph, repo_root, changed_files=None):
    del graph
    findings = []
    for rel_path, required_tokens in sorted(REQUIRED_FILES.items()):
        text = _read_text(repo_root, rel_path)
        if not text:
            findings.append(
                make_finding(
                    analyzer_id=ANALYZER_ID,
                    category="geometry.nondeterministic_distance_smell",
                    severity="RISK",
                    confidence=0.97,
                    file_path=rel_path,
                    line=1,
                    evidence=["required GEO metric/proof file is missing"],
                    suggested_classification="TODO-BLOCKED",
                    recommended_action="RESTORE",
                    related_invariants=["INV-METRIC-PROFILE-DECLARED", "INV-GEO-BUDGETED"],
                    related_paths=[rel_path],
                )
            )
            continue
        missing = [token for token in required_tokens if token not in text]
        if not missing:
            continue
        findings.append(
            make_finding(
                analyzer_id=ANALYZER_ID,
                category="geometry.nondeterministic_distance_smell",
                severity="RISK",
                confidence=0.94,
                file_path=rel_path,
                line=1,
                evidence=["missing GEO deterministic metric token(s): {}".format(", ".join(missing[:4]))],
                suggested_classification="TODO-BLOCKED",
                recommended_action="REWRITE",
                related_invariants=["INV-METRIC-PROFILE-DECLARED", "INV-GEO-BUDGETED"],
                related_paths=[rel_path, "game/domain/geology/metric/metric_engine.py"],
            )
        )

    candidates = []
    if changed_files:
        for rel_path in list(changed_files or []):
            rel_norm = str(rel_path).replace(os.sep, "/")
            if rel_norm.endswith(".py") and any(rel_norm.startswith(prefix) for prefix in WATCH_PREFIXES):
                candidates.append(rel_norm)
    else:
        candidates = list(_iter_candidate_files(repo_root))

    for rel_path in sorted(set(candidates)):
        text = _read_text(repo_root, rel_path)
        if not text:
            continue
        for line_no, line in enumerate(text.splitlines(), start=1):
            snippet = str(line).strip()
            if not snippet or snippet.startswith("#"):
                continue
            if not any(token in snippet for token in _BANNED_TOKENS):
                continue
            findings.append(
                make_finding(
                    analyzer_id=ANALYZER_ID,
                    category="geometry.nondeterministic_distance_smell",
                    severity="RISK",
                    confidence=0.96,
                    file_path=rel_path,
                    line=line_no,
                    evidence=[
                        "nondeterministic source detected in GEO metric/stress surface",
                        snippet[:160],
                    ],
                    suggested_classification="TODO-BLOCKED",
                    recommended_action="REWRITE",
                    related_invariants=["INV-METRIC-PROFILE-DECLARED", "INV-GEO-BUDGETED"],
                    related_paths=[rel_path, "game/domain/geology/metric/metric_engine.py"],
                )
            )
            break
    return findings
