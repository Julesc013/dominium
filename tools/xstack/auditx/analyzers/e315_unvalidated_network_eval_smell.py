...
WATCH_PREFIXES = (
    "tools/xstack/auditx/analyzers/e315_unvalidated_network_eval_smell.py",
    "tools/xstack/auditx/analyzers/__init__.py",
    "game/domain/logic/network/",
    "tools/xstack/sessionx/process_runtime.py",
)

_ALLOWED_PATHS = {
    "game/domain/logic/network/logic_network_engine.py",
    "game/domain/logic/network/logic_network_validator.py",
    "game/domain/logic/network/instrumentation_binding.py",
    "game/domain/logic/network/__init__.py",
}
_EXECUTION_TOKENS = (
    ...
    if '"process.logic_network_validate"' not in runtime_text:
        findings.append(
            make_finding(
                ...
                related_paths=[runtime_rel, "game/domain/logic/network/logic_network_engine.py"],
            )
        )
        ...
        for name in sorted(files):
            ...
            findings.append(
                make_finding(
                    ...
                    related_invariants=["INV-LOGIC-NETWORK-VALIDATED-BEFORE-EVAL"],
                    related_paths=[rel_path, "game/domain/logic/network/logic_network_validator.py"],
                )
            )
