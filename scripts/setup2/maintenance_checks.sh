...
"$root/scripts/setup2/check_kernel_purity.sh"
"$root/scripts/setup2/doc_lint.sh"
"$root/scripts/setup2/schema_freeze_check.sh"

registry="$root/source/dominium/setup/kernel/src/splat/dsk_splat_registry.cpp"
doc="$root/docs/setup2/SPLAT_REGISTRY.md"
...
if git rev-parse "$base" >/dev/null 2>&1; then
    ...
    if echo "$changed" | rg -q "docs/setup2/TLV_"; then
        doc_changed=1
    fi
