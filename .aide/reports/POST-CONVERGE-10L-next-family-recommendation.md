...
Focused RepoX remains at 51 failures and 5 warnings. The distribution/product proof family accounts for 12 failures, but it is not safe to fix by fabricating `dist/bin` wrapper surfaces in this task. The next actionable families are:

- retired-domain path policy checks against `embodiment/`, `geo/`, `diag/`, and `universe`
- tool hash/audit staleness for `tools/compatx/compatx.py`, `tools/securex/securex.py`, and identity fingerprint evidence
