...
- `INV-REFINEMENT-BUDGETED`: MW-4 stress fixture cannot evaluate because `game.domains.embodiment` reaches stale `embodiment.*` lazy imports.
...
- `INV-NO-RANDOM-RETRY-LOOPS-IN-WORLDGEN`: `while True` remains in `game/domains/worldgen/mw/mw_system_refiner_l2.py`.
- `INV-SHADOW-BOUNDED`: bounded shadow policy still flags `while ` in `game/domains/worldgen/earth/lighting/horizon_shadow_engine.py`.
