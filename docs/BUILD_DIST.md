...
- `dominium_dist_init(DIST_ROOT <path> [DIST_OS <token>] [DIST_ARCH <token>] [DIST_VARIANT <token>])`
- `dominium_dist_set_role(target ROLE <launch|game|engine|rend|tools|share>)`
...
cmake --build build/msvc --config Release --target dist_meta
cmake --build build/msvc --config Release --target validate_dist
```
...
cmake --build build/mingw --target dist_meta
cmake --build build/mingw --target validate_dist
```
...
Use the `validate_dist` target to enforce the spec:
```
cmake --build <builddir> --target validate_dist
```
