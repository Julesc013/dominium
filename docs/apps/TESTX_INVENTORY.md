...
## Targets (CMake)
- Client: `dominium_client` -> `client` (`apps/client/CMakeLists.txt`).
- Server: `dominium_server` -> `server` (`apps/server/CMakeLists.txt`).
- Launcher: `launcher_cli` -> `launcher`, plus `launcher_gui`, `launcher_tui` (`apps/launcher/CMakeLists.txt`).
- Setup: `setup_cli` -> `setup`, plus `setup_gui`, `setup_tui` (`apps/setup/CMakeLists.txt`).
...
- Engine tests: `engine_smoke`, performance/IR tests (`tests/engine/CMakeLists.txt`).
- Game tests: many simulation/contract tests and `domino_core_smoke` (`tests/game/CMakeLists.txt`).
...
- Build/version header: generated by `release/packaging/setup/scripts/update_build_number.cmake`.
...
- `runtime/platform/system/sys.c` implements `dsys` with a single compiled backend and a null fallback.
...
- `runtime/render/d_gfx.c` implements backend selection, IR tracing, and registry wiring.
...
- Selection policy enforces explicit failure and auto-selection logging; interfaces documented under `docs/runtime/render/`.
