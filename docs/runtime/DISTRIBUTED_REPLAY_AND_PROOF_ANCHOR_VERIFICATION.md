...
- `contracts/registry/net_replication_policy_registry.json` freezes named replication-policy precursors, but only as provisional policy substrate
- `contracts/registry/provenance_classification_registry.json` and `contracts/registry/provenance_event_type_registry.json` freeze evidence-layer distinctions that later proof packets must respect
