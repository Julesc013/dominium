...
| `doctrine_specification` | Normative semantic, runtime, contract, governance, or policy specification work. | planning-only unless prompt explicitly targets implementation | `specs/**`, `docs/runtime/**`, `docs/reference/contracts/**`, canonical governance docs |
| `governance_policy` | Governance, mirror, instruction, review, refusal, and operator/agent policy work. | planning-only | `AGENTS.md`, `.agentignore`, `docs/game/agents/**`, `data/agents/**` |
