...
- `tools/xstack/controlx/core/execution_router.py` explicitly distinguishes dry-run from live routing and remediation-aware gate orchestration
- `tools/xstack/controlx/core/queue_runner.py` records canonical run logs and escalation outcomes, proving that execution posture is already more than "just run it"
- `tools/release/update_resolver.py` performs governed selection and rollback-oriented transaction logging, proving that release-control actions already carry typed consequences
- `contracts/repo/release_policy.toml` constrains branch, kind, and channel combinations, proving that even basic release movement is policy-shaped
- `contracts/registry/trust_policy_registry.json` is still provisional and `contracts/registry/trust_root_registry.json` is empty, proving that some trust-bearing actions are still constitutionally immature
...
- thin operational roots such as `tools/xstack/controlx/`, `tools/xstack/`, `release/`, `updates/`, or `security/trust/` are important evidence surfaces, but none of them alone becomes execution canon by convenience
