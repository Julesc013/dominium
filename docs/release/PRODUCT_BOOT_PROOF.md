...
| setup | `tools/package/setup/setup_cli.py` | Python setup AppShell bridge | cli | `python tools/package/setup/setup_cli.py --help` | fail | blocked | `TypeError: unsupported operand type(s) for \|: 'type' and 'NoneType'` | local Python 3.8 cannot evaluate the Python 3.10 union annotation in this script |
...
| launcher | `tools/package/launcher/launch.py` | Python launcher AppShell bridge | cli | `python tools/package/launcher/launch.py --help` | pass | partial | AppShell help for `product_id: launcher` | script help boot only; native product binary not proven |
...
| client | `tools/release/mvp/runtime_entry.py` | Python MVP runtime entry | cli | `python tools/release/mvp/runtime_entry.py client --help` | pass | partial | AppShell help for `product_id: client` | script proof only |
...
| server | `tools/release/mvp/runtime_entry.py` | Python MVP runtime entry | cli | `python tools/release/mvp/runtime_entry.py server --help` | pass | partial | AppShell help for `product_id: server` | script proof only |
...
The C client product target is declared, but no built `client` binary exists locally. Both the tracked `archive/generated/dist/bin/dominium_client` wrapper and `tools/release/mvp/runtime_entry.py client --help` emit deterministic AppShell help for `product_id: client`. Client has partial wrapper/script help proof only.

### server

The C server product target is declared, but no built `server` binary exists locally. The tracked `archive/generated/dist/bin/dominium_server` wrapper and `tools/release/mvp/runtime_entry.py server --help` emit deterministic AppShell help for `product_id: server`. Direct `apps/server/server_main.py --help` still enters the default TUI path because script invocation does not forward CLI args. Server has partial wrapper/script help proof only.
