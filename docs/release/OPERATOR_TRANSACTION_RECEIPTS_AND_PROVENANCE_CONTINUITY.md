...
The repo already contains deterministic install transaction logging in `tools/release/update_resolver.py` and historical archive-index retention rules in `docs/release/ARCHIVE_AND_RETENTION_POLICY.md`, but those surfaces remain evidence of continuity practice rather than the full constitutional receipt model by themselves.
...
- `tools/release/update_resolver.py` records deterministic install transactions and exposes rollback-selection helpers, proving that transaction history already matters
...
- `contracts/registry/provenance_classification_registry.json` distinguishes canonical provenance artifacts from derived explain artifacts, proving that the repo already treats provenance classes as layered, not flat
- `contracts/registry/refusal_code_registry.json`, `contracts/registry/refusal_to_exit_registry.json`, and `contracts/registry/remediation_playbooks.json` preserve refusal and remediation meaning, proving that blocked outcomes also need structured evidence
