...
- `tools/release/update_resolver.py` records deterministic transaction anchors such as `transaction_id`, `install_plan_hash`, `prior_component_set_hash`, `selected_component_ids`, `trust_policy_id`, and `trust_result`
...
- `contracts/registry/provenance_classification_registry.json` and `contracts/registry/provenance_event_type_registry.json` already preserve a layered provenance vocabulary rather than flattening evidence into one log stream
