...
- Current status: build proven, targeted AuditX fixed, CTest blocked by unit/RepoX gates
...
| Local status | configure and build pass; focused AuditX tests pass; full CTest fails/times out on unit and RepoX gates |
...
- CTest proof is not complete: focused AuditX tests now pass, but full CTest fails on `invariant_units_present`, fails on `inv_repox_rules`, and exceeds the local 40-minute shell timeout.
...
- FAST still fails after the structural fix because RepoX exposes broad drift and missing-artifact backlog; canonical CTest now exposes the same broad RepoX blocker through `inv_repox_rules`.
