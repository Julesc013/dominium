...
`client/`, `server/`, `setup/`, `launcher/`, `schema/`, `schemas/`, `lib/`, `libs/`, `app/`, `appshell/`, `runtime/`, `contracts/`, `content/`, `archive/`, `attic/`, `legacy/`, `quarantine/`, `geo/`, `chem/`, `fluid/`, `materials/`, `physics/`, `thermal/`, `electric/`, `worldgen/`, `field/`, `fields/`, `process/`, `dist/`, `build/`, `out/`, `artifacts/`, `repo/`.
...
| generated_or_ephemeral_reference | 4 | `build/`, `out/`, `dist/`, and `artifacts/` remain documented as generated or exception-backed. |
...
| `docs/appshell/UI_MODE_RESOLUTION.md` | mode claims | current_path_instruction | cross_reference_added | Product mode support posture linked to component matrix without changing semantics. |
...
| repo-wide generated references | `build/`, `out/`, `dist/`, `artifacts/` | generated_or_ephemeral_reference | not_current_authority | Covered by root policy, exception ledger, and distribution projection docs. |
