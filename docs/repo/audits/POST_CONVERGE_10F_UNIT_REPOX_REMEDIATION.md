...
| --- | --- | --- | --- |
| `unit.mass_energy.stub` | `contracts/schemas/quantity.schema.json`, `data/registries/quantity_registry.json`, `tools/xstack/sessionx/universe_physics.py` | valid provisional unit missing from machine-readable unit registry | added to `data/registries/unit_registry.json` |
| `unit.schema` | path fragment `contracts/schemas/materials/unit.schema` | stale tokenization / false-positive path fragment | `tests/contract/unit_annotation_validation.py` now ignores unit-like tokens preceded by `/` or `\` |
