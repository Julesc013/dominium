...
- What major roots would conceptually become:
  `appshell/`, `client/`, and `server/` stay the main product-shell and authority roots; `tools/` stays a tool umbrella but with sharper internal ownership for `tools/launcher/`, `tools/setup/`, `tools/xstack/`, and `tools/validators/suite/`; `release/`, `security/`, and `repo/` stay the control-plane cluster; `packs/`, `profiles/`, `locks/`, `data/session_templates/`, and `data/registries/` stay the authored data/config family; `docs/`, `specs/`, `schema/`, and `schemas/` stay governance/contract roots with their current split cautions still explicit.
