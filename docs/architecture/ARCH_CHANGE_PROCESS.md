...
## Notes
- Changes that alter invariants must follow `docs/architecture/CHANGE_PROTOCOL.md`.
- Architectural review is mandatory before merge.
- Refusal is valid if invariants cannot be preserved.
- Code reviews MUST include ENTROPY0 checks (`docs/architecture/ANTI_ENTROPY_RULES.md`).
