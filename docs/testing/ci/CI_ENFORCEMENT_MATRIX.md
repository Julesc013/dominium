...
| TERRAIN0-TRUTH-001 | Terrain truth MUST be field-defined SDF; meshes are non-authoritative. | TestX `const_terrain_contracts` | Test | CI fail: TERRAIN0-TRUTH-001 | Restore terrain truth docs/schemas to require `terrain.phi`. | Prevents mesh-authoritative terrain. |
| TERRAIN0-PROC-002 | Terrain overlays MUST be Process-only with provenance. | TestX `const_terrain_contracts` | Test | CI fail: TERRAIN0-PROC-002 | Require process_id/provenance on overlays and fixtures. | Enforces process-only mutation. |
| TERRAIN0-NOSPECIAL-003 | Terrain schema MUST NOT special-case planet/station bodies. | TestX `const_terrain_contracts` | Test | CI fail: TERRAIN0-NOSPECIAL-003 | Remove planet/station specific fields from terrain schemas. | Keeps bodies generic and composable. |
| TERRAIN0-DECAY-004 | Per-tick global erosion is FORBIDDEN. | TestX `const_terrain_contracts` + `inv_terrain_authority` | Test | CI fail: TERRAIN0-DECAY-004 | Route erosion/decay through scheduled Processes only. | Preserves determinism and budgets. |
| TERRAIN0-PHI-005 | Collision/truth checks must reference phi/SDF in terrain code. | TestX `inv_terrain_authority` | Test | CI fail: TERRAIN0-PHI-005 | Route terrain truth queries through `terrain.phi`. | Prevents phi bypass. |
...
- ID alias conflict: `DET-GATE-*` (docs/governance/policies/DETERMINISM_ENFORCEMENT.md) and `DET-G*` (docs/governance/policies/DETERMINISM_GATES.md) describe the same gates. Align to one canonical ID set.
