...
- setup or launcher output in `tools/package/setup/setup_cli.py` and `tools/package/launcher/launch.py`
- negotiation and validation reports in `tools/validators/compatibility/capability_negotiation.py` and `tools/validators/suite/validation_engine.py`
...
- `appshell/command_registry.py`, `tools/package/launcher/launch.py`, and `tools/package/setup/setup_cli.py` prove that command surfaces package integration behavior, install/update defaults, trust wiring, and activation flow around deeper profile declarations. That is why command surfaces stay outside the portable profile contract.
- `tools/validators/suite/validation_engine.py` proves that negotiation and validation suites consume descriptors and negotiation records as evidence-bearing outputs. That supports explicit evidence/review linkage while keeping those reports derived.
