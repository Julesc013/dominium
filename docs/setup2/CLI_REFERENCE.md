...
- `request make --manifest <file> --op <install|upgrade|repair|uninstall|verify|status> --scope <user|system|portable> --ui-mode <cli|tui|gui>`
  ...
  - `--root <path>` optional
  - `--frontend-id <id>` optional (defaults to `dominium-setup2-cli`)
...
## Examples
- `dominium-setup2 request make --manifest manifest.tlv --op install --scope system --ui-mode cli --frontend-id dominium-setup2-cli --out-request request.tlv`
- `dominium-setup2 plan --manifest manifest.tlv --request request.tlv --out-plan plan.tlv`
- `dominium-setup2 apply --plan plan.tlv --out-state installed_state.tlv --out-audit setup_audit.tlv --out-journal job_journal.tlv --dry-run`
- `dominium-setup2 dump-splats --json`
