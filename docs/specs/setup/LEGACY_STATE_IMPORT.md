...
## Kernel API
- `dsk_import_legacy_state` consumes legacy bytes and emits Setup state + audit.
- Deterministic mode sets `run_id=0`.

## CLI
```
dominium-setup import-legacy-state --in <legacy_state> --out <installed_state> --out-audit <setup_audit.tlv> --deterministic 1
```
