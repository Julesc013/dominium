...
This document locks the **v1 JSON contract** for `dominium-setup-legacy`.
...
- `invocation_digest64` is computed from the canonical invocation payload
  (see `docs/setup_legacy/INVOCATION_PAYLOAD.md`).
...
`details.manifest` is the canonical JSON manifest object (see `docs/setup_legacy/MANIFEST_SCHEMA.md`).
...
The exported JSON log file schema is documented in `docs/setup_legacy/AUDIT_LOG_FORMAT.md`.
