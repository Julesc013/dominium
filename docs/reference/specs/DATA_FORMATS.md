...
All supported runtime targets are little-endian. Deterministic artifacts are integer/fixed-point-only; unknown sections are skipped, not rejected. For ABI-stable containers and explicit parsing rules, see `docs/reference/specs/SPEC_CONTAINER_TLV.md`.
...
- ABI-stable containers should use `DTLV` (`docs/reference/specs/SPEC_CONTAINER_TLV.md`).
