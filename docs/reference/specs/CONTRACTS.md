...
- Does not replace subsystem specs under `docs/reference/specs/SPEC_*.md`; those specs remain
...
- If a struct is intended for cross-module ABI use, it must be **versioned and
  size-stamped** as described in `docs/reference/specs/SPEC_ABI_TEMPLATES.md`.
- Do not rely on raw `memcmp()` of public structs for determinism or hashing
  unless the spec explicitly permits it (`docs/reference/specs/SPEC_DETERMINISM.md`).
...
- Determinism requirements are subsystem-specific:
  - Deterministic simulation code must follow `docs/reference/specs/SPEC_DETERMINISM.md`.
