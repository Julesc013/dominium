...
- Client (`apps/client/app/main_client.c`): `--help`, `--version`, `--build-info`, `--status`, `--smoke`, `--selftest`, `--renderer`, window flags (`--windowed`, `--borderless`, `--fullscreen`, `--width`, `--height`), `--control-*`, `--mp0-connect=local`.
- Server (`apps/server/app/main_server.c`): `--help`, `--version`, `--build-info`, `--status`, `--smoke`, `--selftest`, `--control-*`, `--mp0-loopback`, `--mp0-server-auth`.
...
- Build/version header: generated by `apps/setup/packages/scripts/update_build_number.cmake`.
