...
- `high` `conflicting_definition`: UI mode selection rules diverge between the older app doc and the frozen AppShell order. [`docs/apps/UI_MODES.md`, `docs/runtime/shell/APPSHELL_CONSTITUTION.md`, `docs/release/FROZEN_INVARIANTS_v0_0_0.md`]
...
- `medium` `validation unification` -> `docs/testing/validation/VALIDATION_UNIFICATION.md`: RepoX, AuditX, TestX, CompatX, and ARCH-AUDIT surfaces are documented in fragments rather than one convergence doc.
- `medium` `user-facing UI mode selection` -> `docs/apps/UI_MODE_SELECTION.md`: the old UI mode summary is contradictory and the current mode order only appears indirectly in AppShell and release-freeze docs.
