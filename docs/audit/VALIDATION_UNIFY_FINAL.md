...
- `validate.registries` -> `refused` via `stability_registry_suite` (errors=4, warnings=0)
...
- `validate.library` -> `complete` via `library_manifest_suite` (errors=0, warnings=0)
- `validate.time_anchor` -> `complete` via `time_anchor_suite` (errors=0, warnings=0)
- `validate.arch_audit` -> `refused` via `arch_audit_suite` (errors=4, warnings=9)
