...
### Docs beyond canon
- `docs/architecture/` contains prior canonical-style docs from earlier prompts.
...
- **Dual canon trees**: `docs/arch/` is declared canonical by EG-H, but `docs/architecture/` contains overlapping authoritative docs. These may conflict and require archival or clear cross-linking.
...
3. **Canon consolidation**: Identify conflicts between `docs/arch/` and `docs/architecture/`; archive or cross-link legacy docs.
...
Phase 0 is complete. The repo contains the required canonical docs in `docs/arch/`, but TESTX, version governance, and public header hygiene are missing or ambiguous. These gaps must be addressed in later phases without weakening canon.
