...
| --- | --- | --- | --- | --- |
| Repo-local bootstrap | build_verify_lane<br>appshell_bootstrap<br>release_control_plane | None if existing verify binaries are acceptable; otherwise run cmake --preset verify and build the preset. | Use verify preset plus repo_wrapper_shim archive/generated/dist/ manifest. | verify configure succeeds<br>verify binaries exist or are rebuilt |
