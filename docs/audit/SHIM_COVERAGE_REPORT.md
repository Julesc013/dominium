...
| `tools/package/distribution/distribution_lib.py` | `integrated` | `redirect_legacy_path(` |
...
- surface_id=`legacy.compatx_cli` path=`tools/xstack/compatx/compatx.py` status=`active_adapter` replacement_target=`src/validation/validation_engine.py`
...
- surface_id=`legacy.graph_validate` path=`tools/validators/core/tool_graph_validate.py` status=`deprecated` replacement_target=`tools/xstack/compatx/check.py`
...
- surface_id=`legacy.hygiene_validate_cli` path=`tools/validators/hygiene_validate_cli.cpp` status=`deprecated` replacement_target=`tools/audit/tool_run_arch_audit.py`
- surface_id=`legacy.validate_all_cpp` path=`tools/validators/suite/validate_all_main.cpp` status=`deprecated` replacement_target=`src/validation/validation_engine.py::validate.determinism`
- surface_id=`legacy.validation_registry_cpp` path=`tools/validators/suite/validators_registry.cpp` status=`deprecated` replacement_target=`data/registries/validation_suite_registry.json`
