# Setup2 Handoff (Launcher)

The launcher consumes Setup2 `installed_state.tlv` and imports legacy state when required.
...
## Import behavior
- If Setup2 state is missing but legacy state exists, the launcher calls `dsk_import_legacy_state`.
