...
The custom action must only call `dominium-setup`:
- `request make`
- `plan`
- `apply`
...
```
msiexec /i DominiumSetup.msi /qn DSK_OPERATION=install DSK_SCOPE=system DSK_INSTALL_ROOT="C:\\Dominium" /l*v setup_msi.log
```
