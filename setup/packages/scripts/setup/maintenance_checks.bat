...
call "%ROOT%\scripts\setup\check_kernel_purity.bat" || exit /b 1
call "%ROOT%\scripts\setup\doc_lint.bat" || exit /b 1
call "%ROOT%\scripts\setup\schema_freeze_check.bat" || exit /b 1

set REG=%ROOT%\source\dominium\setup\kernel\src\splat\dsk_splat_registry.cpp
set DOC=%ROOT%\docs\setup\SPLAT_REGISTRY.md
set FAIL=0
