...
def main() -> int:
    parser = argparse.ArgumentParser(description="Invariant: no anonymous RNG usage in authoritative code.")
    parser.add_argument("--repo-root", default=".")
    args = parser.parse_args()
    repo_root = os.path.abspath(args.repo_root)

    invariant_id = "INV-DET-NO-ANON-RNG"
        ...
        for path in iter_files([root], DEFAULT_EXCLUDES, SOURCE_EXTS):
            ...
            for idx, line in enumerate(stripped.splitlines(), start=1):
                if FORBIDDEN_RNG_CALL_RE.search(line):
                    violations.append("{}:{}: anonymous RNG call".format(rel, idx))

    if violations:
        print("{}: anonymous RNG usage detected".format(invariant_id))
        for item in sorted(violations):
            print(item)
        return 1

    print("no anonymous RNG usage detected")
    return 0
