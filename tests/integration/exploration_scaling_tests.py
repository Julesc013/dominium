import argparse
import hashlib
import json
import os
import shutil
import subprocess
import sys


REPLAY_HEADER = "DOMINIUM_REPLAY_V1"


def run_cmd(cmd, expect_code=0, expect_contains=None, cwd=None, env=None):
    result = subprocess.run(
        cmd,
        stdout=subprocess.PIPE,
        stderr=subprocess.STDOUT,
        text=True,
        errors="replace",
        cwd=cwd,
        env=env,
    )
    output = result.stdout or ""
    if expect_code is not None and result.returncode != expect_code:
        sys.stderr.write("FAIL: expected exit {} for {}\n".format(expect_code, cmd))
        sys.stderr.write(output)
        return False, output
    if expect_contains:
        for token in expect_contains:
            if token not in output:
                sys.stderr.write("FAIL: missing '{}' in output for {}\n".format(token, cmd))
                sys.stderr.write(output)
                return False, output
    return True, output


def ensure_clean_dir(path):
    if os.path.isdir(path):
        shutil.rmtree(path, ignore_errors=True)
    os.makedirs(path, exist_ok=True)


def read_lines(path):
    with open(path, "r", encoding="utf-8", errors="replace") as handle:
        return [line.rstrip("\n\r") for line in handle]


def read_text(path):
    if not os.path.isfile(path):
        return ""
    with open(path, "r", encoding="utf-8", errors="replace") as handle:
        return handle.read()


def sha256_file(path):
    h = hashlib.sha256()
    with open(path, "rb") as handle:
        for chunk in iter(lambda: handle.read(8192), b""):
            h.update(chunk)
    return h.hexdigest()


def require(condition, message):
    if not condition:
        sys.stderr.write("FAIL: {}\n".format(message))
        return False
    return True


def parse_event_lines(path):
    lines = read_lines(path)
    events = []
    in_events = False
    saw_events = False
    for line in lines:
        if line == REPLAY_HEADER:
            continue
        if line == "events_begin":
            in_events = True
            saw_events = True
            continue
        if line == "events_end":
            in_events = False
            continue
        if (saw_events and in_events) or (not saw_events and line):
            events.append(line)
    return events


def parse_status_block(text):
    data = {}
    for line in text.splitlines():
        if "=" not in line:
            continue
        key, value = line.split("=", 1)
        data[key.strip()] = value.strip()
    return data


def count_token(lines, token):
    return sum(1 for line in lines if token in line)


def count_text_token(text, token):
    return text.count(token)


def load_fixture_config(repo_root):
    cfg_path = os.path.join(repo_root, "tests", "perf", "exploration_fixtures", "fixtures.json")
    with open(cfg_path, "r", encoding="utf-8") as handle:
        return json.load(handle)


def select_renderers(client_path):
    ok, output = run_cmd([client_path, "capabilities"], expect_code=None)
    if not ok or not output:
        return ["null"]
    renderers = []
    for line in output.splitlines():
        if not line.startswith("renderer="):
            continue
        parts = line.split()
        head = parts[0]
        name = head.split("=", 1)[1] if "=" in head else ""
        supported = "supported=1" in line
        if name and supported:
            renderers.append(name)
    if not renderers:
        renderers = ["null"]
    if "null" in renderers:
        renderers = ["null"] + [r for r in renderers if r != "null"]
    return renderers


def main():
    parser = argparse.ArgumentParser(description="Exploration scaling integration tests.")
    parser.add_argument("--client", required=True)
    parser.add_argument("--temp-root", required=True)
    parser.add_argument("--repo-root", required=True)
    args = parser.parse_args()

    client_path = os.path.abspath(args.client)
    temp_root = os.path.abspath(args.temp_root)
    repo_root = os.path.abspath(args.repo_root)

    ok = True
    ok = ok and require(os.path.isfile(client_path), "client binary missing")
    if not ok:
        return 1

    cfg = load_fixture_config(repo_root)
    nav_template = cfg.get("nav_script_template", "")
    status_template = cfg.get("status_script_template", "")
    ui_script = cfg.get("ui_script", "")
    fixtures = cfg.get("fixtures", [])

    ok = ok and require(fixtures, "fixtures list missing")
    ok = ok and require(nav_template, "nav_script_template missing")
    ok = ok and require(status_template, "status_script_template missing")
    ok = ok and require(ui_script, "ui_script missing")
    if not ok:
        return 1

    renderers = select_renderers(client_path)
    renderer_set = renderers[:2] if len(renderers) > 1 else renderers

    metrics_by_fixture = {}
    replay_hashes = []
    ui_logs_by_fixture = {}

    for entry in fixtures:
        fixture_id = entry.get("id", "")
        root = entry.get("root", "")
        if not fixture_id or not root:
            ok = ok and require(False, "fixture entry missing id/root")
            continue

        fixture_dir = os.path.join(repo_root, "tests", "perf", "exploration_fixtures", root)
        data_root = os.path.join(temp_root, root, "data")
        saves_dir = os.path.join(data_root, "saves")
        replays_dir = os.path.join(data_root, "replays")
        ensure_clean_dir(data_root)
        os.makedirs(saves_dir, exist_ok=True)
        os.makedirs(replays_dir, exist_ok=True)

        env = dict(os.environ)
        env["DOM_INSTALL_ROOT"] = fixture_dir
        env["DOM_DATA_ROOT"] = data_root
        env["DOM_INSTANCE_ROOT"] = data_root

        ok = ok and run_cmd(
            [client_path, "create-world template=world.template.exploration_baseline seed=42"],
            expect_contains=["world_create=ok", "world_save=ok"],
            cwd=data_root,
            env=env,
        )[0]
        if not ok:
            continue

        save_files = [name for name in os.listdir(saves_dir) if name.endswith(".save")]
        ok = ok and require(save_files, "no saves produced for {}".format(fixture_id))
        if not ok:
            continue

        original_save = os.path.join(saves_dir, save_files[0])
        canonical_save = os.path.join(saves_dir, "world.save")
        shutil.copy2(original_save, canonical_save)

        save_rel = os.path.join("saves", "world.save")
        save2_rel = os.path.join("saves", "exploration.save")
        replay_rel = os.path.join("replays", "exploration.replay")
        save2_path = os.path.join(saves_dir, "exploration.save")
        replay_path = os.path.join(replays_dir, "exploration.replay")

        nav_script = nav_template.format(
            save_path=save_rel,
            save2_path=save2_rel,
            replay_path=replay_rel,
        )
        batch_cmd = "batch {}".format(nav_script)
        ok = ok and run_cmd(
            [client_path, batch_cmd],
            expect_contains=["world_save=ok", "replay_save=ok"],
            cwd=data_root,
            env=env,
        )[0]

        ok = ok and require(os.path.isfile(save2_path), "save missing for {}".format(fixture_id))
        ok = ok and require(os.path.isfile(replay_path), "replay missing for {}".format(fixture_id))
        if not ok:
            continue

        status_script = status_template.format(save_path=save_rel)
        status_cmd = "batch {}".format(status_script)
        status_ok, status_out = run_cmd(
            [client_path, status_cmd],
            cwd=data_root,
            env=env,
        )
        ok = ok and status_ok

        camera_cmd = "batch load path={}; camera-next; where".format(save_rel)
        camera_ok, camera_out = run_cmd(
            [client_path, camera_cmd],
            cwd=data_root,
            env=env,
        )
        ok = ok and camera_ok

        inspect_cmd = "batch load path={}; inspect-toggle; where".format(save_rel)
        inspect_ok, inspect_out = run_cmd(
            [client_path, inspect_cmd],
            cwd=data_root,
            env=env,
        )
        ok = ok and inspect_ok

        if not ok:
            continue

        status_data = parse_status_block(status_out)
        camera_data = parse_status_block(camera_out)
        inspect_data = parse_status_block(inspect_out)

        ok = ok and require(status_data.get("current_node_id") == "body.earth",
                            "current_node_id mismatch in {}".format(fixture_id))
        ok = ok and require(camera_data.get("position") == status_data.get("position"),
                            "camera movement changed position in {}".format(fixture_id))
        ok = ok and require(inspect_data.get("current_node_id") == "body.earth",
                            "inspect changed domain in {}".format(fixture_id))

        events = parse_event_lines(replay_path)
        ok = ok and require(events, "replay events missing for {}".format(fixture_id))
        if not ok:
            continue

        event_hash = sha256_file(replay_path)
        replay_hashes.append(event_hash)

        ui_logs = {}
        for renderer in renderer_set:
            log_path = os.path.join(temp_root, root, "ui_{}.log".format(renderer))
            ok = ok and run_cmd(
                [
                    client_path,
                    "--ui=gui",
                    "--headless",
                    "--renderer",
                    renderer,
                    "--ui-log",
                    log_path,
                    "--ui-script",
                    ui_script,
                    "--ui-frames",
                    "24",
                ],
                cwd=data_root,
                env=env,
            )[0]
            ui_logs[renderer] = read_text(log_path)

        ui_logs_by_fixture[fixture_id] = ui_logs

        renderer_draw_calls = 0
        renderer_traversal_nodes = 0
        if ui_logs:
            sample_text = next(iter(ui_logs.values()), "")
            renderer_draw_calls = count_text_token(sample_text, "renderer.draw")
            renderer_draw_calls += count_text_token(sample_text, "ui.render")
            renderer_traversal_nodes = count_text_token(sample_text, "node_id=")
            renderer_traversal_nodes += count_text_token(sample_text, "topology.")

        metrics = {
            "active_domain_count": 1 if status_data.get("current_node_id") else 0,
            "expanded_domain_count": 1 if status_data.get("current_node_id") else 0,
            "camera_update_ops_per_tick": count_token(events, "client.nav.camera"),
            "inspect_ops_per_tick": count_token(events, "client.inspect.toggle"),
            "renderer_draw_calls": renderer_draw_calls,
            "renderer_traversal_nodes": renderer_traversal_nodes,
            "snapshot_reads_per_tick": count_token(events, "client.world.load"),
            "event_hash": event_hash,
        }
        metrics_by_fixture[fixture_id] = metrics

        # No collapse/expand events should appear in exploration shell logs.
        ok = ok and require(count_token(events, "collapse") == 0,
                            "collapse event seen in {}".format(fixture_id))
        ok = ok and require(count_token(events, "expand") == 0,
                            "expand event seen in {}".format(fixture_id))

    if not ok:
        return 1

    base_metrics = None
    for fixture_id, metrics in metrics_by_fixture.items():
        if base_metrics is None:
            base_metrics = metrics
            continue
        for key, value in base_metrics.items():
            if key == "event_hash":
                continue
            if metrics.get(key) != value:
                sys.stderr.write("FAIL: metric mismatch {} for {} ({} != {})\n".format(
                    key, fixture_id, metrics.get(key), value))
                ok = False

    if len(set(replay_hashes)) > 1:
        sys.stderr.write("FAIL: replay hash mismatch across fixtures\n")
        ok = False

    # Renderer parity: logs must match across renderers (if more than one).
    if len(renderer_set) > 1:
        for fixture_id, logs in ui_logs_by_fixture.items():
            if len(logs) < 2:
                continue
            log_values = list(logs.values())
            if log_values[0] != log_values[1]:
                sys.stderr.write("FAIL: renderer logs differ in {}\n".format(fixture_id))
                ok = False

    return 0 if ok else 1


if __name__ == "__main__":
    raise SystemExit(main())
