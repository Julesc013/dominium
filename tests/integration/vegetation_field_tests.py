...
def main():
    parser = argparse.ArgumentParser(description="Vegetation T5 integration tests.")
    ...
    fixture_root = os.path.join(repo_root, "tests", "fixtures", "vegetation")

    fixtures = [
        "planet_like.vegetation",
        "oblate_world.vegetation",
        "superflat_slab.vegetation",
        "asteroid_tiny.vegetation",
    ]

    ok = True
    ok = ok and require(os.path.isfile(tool_path), "vegetation tool missing")
    ok = ok and require(os.path.isdir(fixture_root), "vegetation fixtures missing")
    ...
    for fixture in fixtures:
        ...
        success, _output = run_cmd(
            ...
            expect_contains=["DOMINIUM_VEGETATION_VALIDATE_V1", "provider_chain=terrain->climate->weather->geology->vegetation"],
        )
        ok = ok and success

    planet_fixture = os.path.join(fixture_root, "planet_like.vegetation")
    success, output = run_cmd(
        ...
        expect_contains=["DOMINIUM_VEGETATION_INSPECT_V1", "provider_chain=terrain->climate->weather->geology->vegetation"],
    )
    ok = ok and success
    if success:
    ...
    success, output = run_cmd(
        ...
        expect_contains=["DOMINIUM_VEGETATION_INSPECT_V1"],
    )
    ok = ok and success
    if success:
        data = parse_kv(output)
    ...
    success, output = run_cmd(
        ...
        expect_contains=["DOMINIUM_VEGETATION_CORE_SAMPLE_V1", "provider_chain=terrain->climate->weather->geology->vegetation"],
    )
    ok = ok and success
    sample_hash = None
    if success:
    ...
    success, output = run_cmd(
        ...
        expect_contains=["DOMINIUM_VEGETATION_CORE_SAMPLE_V1"],
    )
    ok = ok and success
    ...
    other_fixture = os.path.join(fixture_root, "oblate_world.vegetation")
    success, output = run_cmd(
        [
            tool_path,
            "diff",
        ...
        expect_contains=["DOMINIUM_VEGETATION_DIFF_V1"],
    )
    ok = ok and success
    if success:
        data = parse_kv(output)
    ...
    success, output = run_cmd(
        ...
        expect_contains=["DOMINIUM_VEGETATION_COLLAPSE_V1"],
    )
    ok = ok and success
    if success:
        data = parse_kv(output)
