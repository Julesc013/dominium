...
def main():
    parser = argparse.ArgumentParser(description="Structure T7 perf/scaling tests.")
    ...
    success, output = run_cmd(
        ...
        expect_contains=["DOMINIUM_STRUCTURE_RENDER_V1"],
    )
    ok = ok and success
    if success:
        data = parse_kv(output)
        ...
        ok = ok and require(
            data.get("visible_structures") == data.get("touched_structures"),
            "render touched invisible structures",
        )
