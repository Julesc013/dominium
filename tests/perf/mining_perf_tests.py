...
def main():
    parser = argparse.ArgumentParser(description="Mining T9 perf/scaling tests.")
    ...
    if success:
        data = parse_kv(output)
        cut_cost = int(data.get("cost_step_max", "0"))
        ok = ok and require(cut_cost == baseline_cost, "cuts changed per-step cost")
