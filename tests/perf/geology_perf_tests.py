...
def main():
    parser = argparse.ArgumentParser(description="Geology T2 perf/scaling tests.")
    ...
    if success:
        data = parse_kv(output)
        ok = ok and require(data.get("capsule_count") == "1", "collapsed tile expanded unexpectedly")
