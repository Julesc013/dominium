...
#ifndef SETUP2_SOURCE_DIR
#define SETUP2_SOURCE_DIR "."
#endif
...
static std::string root_path(const char *suffix) {
    std::string root = SETUP2_SOURCE_DIR ? SETUP2_SOURCE_DIR : ".";
...
static int test_kernel_invariants_match_launcher(void) {
    static const char *k_setup2_markers[] = {
    ...
    if (require_markers(root_path("docs/setup2/INVARIANTS.md"),
                        k_setup2_markers,
                        sizeof(k_setup2_markers) / sizeof(k_setup2_markers[0])) != 0) {
        return 1;
    }
...
static int test_capability_registry_semantics_match(void) {
    ...
    if (require_markers(root_path("docs/setup2/SPLAT_SELECTION_RULES.md"),
                        k_setup2_markers,
                        sizeof(k_setup2_markers) / sizeof(k_setup2_markers[0])) != 0) {
        return 1;
    }
...
static int test_job_journal_semantics_match(void) {
    ...
    if (require_markers(root_path("docs/setup2/JOB_ENGINE.md"),
                        k_setup2_markers,
                        sizeof(k_setup2_markers) / sizeof(k_setup2_markers[0])) != 0) {
        return 1;
    }
...
int main(int argc, char **argv) {
    if (argc < 2) {
        std::fprintf(stderr, "usage: setup2_parity_tests <test>\n");
        return 1;
    }
