...
int main(int argc, char **argv) {
    std::string sandbox_root;
    std::string fixtures_root = "tests/setup2/fixtures";
    std::string out_json;
    ...
    if (sandbox_root.empty()) {
        std::fprintf(stderr, "usage: setup2_conformance_runner --sandbox-root <path> [--fixtures-root <path>] [--deterministic 1] [--out-json <path>]\n");
        return 1;
    }
    (void)deterministic;
...

    {
        ...
        size_t idx;
        json += "{\n";
        json += "  \"schema_version\": \"setup2-conformance-1\",\n";
        json += "  \"cases\": [\n";
