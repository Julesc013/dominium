...
static int check_parity_lock_matrix(const std::string &repo_root) {
    std::string path = join_path(repo_root, "docs/setup2/PARITY_LOCK_MATRIX.md");
...
int main(int argc, char **argv) {
    if (argc < 7) {
        std::fprintf(stderr,
                     "usage: setup2_parity_lock_tests <test> <cli> <tui> <fixtures_root> <sandbox_root> <repo_root> [steam] [win_exe]\n");
        return 1;
    }
