...
int main(int argc, char **argv) {
    if (argc < 5) {
        std::fprintf(stderr,
                     "usage: test_cli_golden <dominium-setup2> <fixtures_root> <golden_root> <sandbox_root> [--update]\n");
        return 1;
    }
    ...
    std::string exe_local = join_path(sandbox_root, "dominium-setup2.exe");
    ...
    if (!copy_file(exe, exe_local)) {
        return fail("failed to stage dominium-setup2");
    }
