...
int main(int argc, char **argv) {
    ...
    }
    {
        ...
        if (!file_contains(postinstall, "dominium-setup2 request make")) {
            return fail("postinstall missing request make");
        }
        if (!file_contains(postinstall, "dominium-setup2 plan")) {
            return fail("postinstall missing plan");
        }
        if (!file_contains(postinstall, "dominium-setup2 apply")) {
