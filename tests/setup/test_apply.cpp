...
#ifndef SETUP_TESTS_SANDBOX_ROOT
#define SETUP_TESTS_SANDBOX_ROOT "."
#endif
...
static int init_services(const char *case_name, dss_services_t *services, std::string &root_out) {
    ...
    std::string root = std::string(SETUP_TESTS_SANDBOX_ROOT) + "/" + case_name;
...
int main(int argc, char **argv) {
    if (argc < 2) {
        std::fprintf(stderr, "usage: setup_apply_tests <test>\n");
        return 1;
    }
