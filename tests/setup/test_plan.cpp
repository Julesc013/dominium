...
static int test_plan_golden_default(void) {
    ...
    dsk_status_t st;
    std::string path = std::string(SETUP_TESTS_SOURCE_DIR) + "/golden/plan_default.tlv";
...
static int test_plan_golden_custom(void) {
    ...
    std::vector<dsk_u8> golden;
    dsk_status_t st;
    std::string path = std::string(SETUP_TESTS_SOURCE_DIR) + "/golden/plan_custom.tlv";
...
int main(int argc, char **argv) {
    if (argc < 2) {
        std::fprintf(stderr, "usage: setup_plan_tests <test>\n");
        return 1;
    }
