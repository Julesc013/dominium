...
def compile_with_gcc_like(compiler, include_root, header_rel):
    src = '#include "{}"\n'.format(header_rel)
...
def compile_with_msvc(compiler, include_root, header_rel):
    src = '#include "{}"\n'.format(header_rel)
    src += "int main() { return 0; }\n"
    with tempfile.TemporaryDirectory() as tmpdir:
        src_path = os.path.join(tmpdir, "header_check.cpp")
...
def main() -> int:
    ...
    for header in headers:
        ...
        if compiler:
            code, output = compile_with_gcc_like(compiler, include_root, rel)
        else:
            code, output = compile_with_msvc(msvc, include_root, rel)
        if code != 0:
