...
def main() -> int:
    ...
    id_rules_doc = os.path.join(repo_root, "docs", "arch", "ID_AND_NAMESPACE_RULES.md")
    ...
    if not unit_rows:
        violations.append("missing units block in docs/arch/UNIT_SYSTEM_POLICY.md")
    else:
        unit_ids = [(row[0], "docs/arch/UNIT_SYSTEM_POLICY.md") for row in unit_rows]
        base_ids = [(row[2], "docs/arch/UNIT_SYSTEM_POLICY.md") for row in unit_rows]
