...
bool dom_sim_caps_to_tlv(const DomSimCaps &caps, DomCapsBuffer &out);
bool dom_sim_caps_from_tlv(const unsigned char *data, u32 len, DomSimCaps &out);
u64 dom_sim_caps_hash64(const DomSimCaps &caps);

bool dom_perf_caps_to_tlv(const DomPerfCaps &caps, DomCapsBuffer &out);
