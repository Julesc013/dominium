/*
FILE: runtime/include/domino/ui/dui/dui_schema_tlv.h
MODULE: DUI
LAYER / SUBSYSTEM: DUI API / schema TLV
RESPONSIBILITY: Defines the TLV tags and enums for DUI contracts/schema/state snapshots (POD-only, baseline-visible).
...
DETERMINISM: Data-driven UI only; contracts/schema/state are inputs and must not influence simulation.
VERSIONING / ABI / DATA FORMAT NOTES: TLV format; skip-unknown; see `docs/reference/specs/SPEC_DUI.md`.
...
/* DUI contracts/schema/state TLV uses the canonical TLV record shape:
...
/* Optional absolute rect (payload = 4x i32: x,y,w,h). */
#define DUI_TLV_RECT_I32         ((u32)0x52454354u) /* 'RECT' */
...
#define DUI_NODE_FLAG_ABSOLUTE  ((u32)1u << 2u)
