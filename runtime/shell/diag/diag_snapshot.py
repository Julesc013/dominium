...
from runtime.shell.logging import append_jsonl
...
def write_diag_snapshot_bundle(
    *,
    ...
    ipc_rows = [dict(_normalize_tree(dict(row))) for row in list(ipc_attach_rows or [])[-max(0, int(log_tail or 0)) :]]
    ...
    ipc_path = _write_json(os.path.join(bundle_dir, "ipc_attach_records.json"), {"ipc_attach_records": ipc_rows})
    ...
    manifest = {
        ...
        "ipc_attach_count": int(len(ipc_rows)),
        "files": {
            ...
            "ipc_attach_records": os.path.basename(ipc_path),
