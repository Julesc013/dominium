...
VIRTUAL_ROOT_REGISTRY_REL = os.path.join("contracts", "registry", "virtual_root_registry.json")
...
def vpath_init(context: Mapping[str, object]) -> dict:
    from runtime.package.install_discovery_engine import discover_install
    ...
    install_discovery = discover_install(
        ...
        cwd=os.getcwd(),
        env=os.environ,
        explicit_registry_path=os.path.join(repo_root, "contracts", "registry", "install_registry.json"),
    )
