...
from runtime.shell.ipc.ipc_transport import remove_ipc_manifest_entry
from runtime.shell.logging import append_jsonl, log_emit
...
SUPERVISOR_SERVICE_SCRIPT_REL = os.path.join("tools", "validators", "shell", "supervisor_service.py")
SUPERVISED_PRODUCT_HOST_REL = os.path.join("tools", "validators", "shell", "supervised_product_host.py")
SUPERVISOR_SERVICE_MODULE = "tools.validators.shell.supervisor_service"
SUPERVISED_PRODUCT_HOST_MODULE = "tools.validators.shell.supervised_product_host"
...
def _verification_root(repo_root: str) -> str:
    ...
    if context is not None:
        ...
        store_contract_registry = _runtime_abs(
            store_root,
            os.path.join("contracts", "registry", "semantic_contract_registry.json"),
        )
        ...
        if resolution_source != "repo_wrapper_shim":
            return _runtime_abs(repo_root, ".")
        return _runtime_abs(repo_root, ".")
    dist_contract_registry = _runtime_abs(repo_root, os.path.join("dist", "contracts", "registry", "semantic_contract_registry.json"))
